...
import {
  mapStatusToStageId, getActiveStages, loadPipelineSettings, PIPELINE_SETTINGS_EVENT
} from "./stageConfig";
...
// ─── Column definitions ───────────────────────────────────────────────────────
// Outcome-shaped columns. Anything describing support that was actually
// delivered holds no value on an application that was declined — what matters
// is what was asked for, when it came in, when it closed, and why.
const DEFAULT_COLUMN_ORDER = [
  "fundingRequired", "supportRequired", "appliedDate", "declinedDate",
  "currentStatus", "reason", "sector", "location", "fundingStage", "servicesRequired"
];

const COLUMN_DEFS = {
  fundingRequired: {
    label: "Funding Requested", minWidth: "132px", filter: "range", sortKey: "fundingAmount",
    tooltip: "The amount of support the business asked for. Filtering and sorting use the underlying number, not the formatted label.",
  },
  supportRequired: {
    label: "Support Required", minWidth: "134px", filter: "select", type: "badge",
    tooltip: "The kind of help the business said it needed — funding, market access, mentoring, technical support and so on.",
  },
  appliedDate: {
    label: "Date Applied", minWidth: "116px", filter: "date", type: "date",
    tooltip: "When the business submitted its application to this programme.",
  },
  declinedDate: {
  ...
  reason: {
    ...
    tooltip: "The message sent when the application was moved to its final stage. Blank where nothing was recorded.",
  },
  sector: {
  ...
  fundingStage: {
    label: "Business Stage", minWidth: "124px", filter: "select",
    tooltip: "Where the business sat in its journey when it applied — startup, growth, scale or established.",
  },
  servicesRequired: {
    label: "Services", minWidth: "140px", filter: "text",
    tooltip: "The specific services the business asked for, in its own words.",
  },
};

const NAME_TOOLTIP = "The registered business name, with its programme number appended when the same business applied to more than one of your programmes. Click the eye to open its full profile.";
const ACTIONS_TOOLTIP = "Open the full profile for this business, including what it applied for.";

const DEFAULT_COLUMN_VISIBILITY = {
  fundingRequired: true, supportRequired: true, appliedDate: true, declinedDate: true,
  currentStatus: true, reason: true, sector: true, location: true,
  fundingStage: false, servicesRequired: false,
};

const EXPORT_HEADERS = {
  sme: "Business Name", fundingRequired: "Funding Requested", supportRequired: "Support Required",
  ...
  fundingStage: "Business Stage", servicesRequired: "Services",
};

// ─── Views ────────────────────────────────────────────────────────────────────
// A "view" bundles column visibility, order, widths, sort and density into one
// named object, with exactly one active at a time. Editing the table edits the
// active view, so there's no hidden layout that can drift out of sync.
const BUILTIN_VIEW_ID = "__default__";
const VIEWS_STORAGE_KEY = "catalyst-declined-deals-views-v1";
...
const DeclinedSupportDealsTable = ({ onCountChange }) => {
  // Rows come from the same portfolio context the pipeline table reads, not a
  // separate Firestore query. The context already holds every application with
  // its profile joined on, so this table can't disagree with the pipeline about
  // what a business is or which stage it ended at.
  ...
  // ─── Rows ───────────────────────────────────────────────────────────────
  // Mapped to the same shape the pipeline table builds, because SMEDetailsModal
  // is handed the row object directly — a thinner row renders a thinner profile.
  const deals = useMemo(() => {
    ...
    const rows = [];
    (enriched || []).forEach((a) => {
      const rawStatus = (a.pipelineStage || a.status || "").toString();
      const stageId = mapStatusToStageId(rawStatus, activeStages);
      ...
      // programme template would otherwise be misread as "Matched" and vanish.
      // The raw-status check catches those.
      ...
      rows.push({
        ...
        rowKey: `${a.smeId}_${a.programIndex}`,
        ...
        appliedDate: a.applicationDate || null,
        applicationDate: a.applicationDate
          ? new Date(a.applicationDate).toLocaleDateString("en-ZA", { month: "short", day: "numeric", year: "numeric" })
          : "N/A",
        // `updatedAt` is written on every stage change, so for a terminal row it
        // is the moment the decline was recorded.
        declinedDate: a.updatedAt || a.applicationDate || null,
        reason: a.lastMessage || a.declineReason || "No reason recorded",
    ...
    return rows;
  }, [enriched, activeStages]);

  useEffect(() => { onCountChange?.(deals.length); }, [deals.length, onCountChange]);
  ...
  const processed = useMemo(() => {
    ...
    if (sortConfig?.key) {
      ...
      const field = isName ? "name" : (col?.sortKey || sortConfig.key);
  ...
  // at all — otherwise rows fall back to context order, which reads as random.
  ...
  const handleExport = () => {
    try {
      ...
      const rows = processed.map((d) => {
        const row = { [EXPORT_HEADERS.sme]: d.name };
        visible.forEach((k) => {
  ...
  const renderCell = (deal, key) => {
    ...
    switch (col.type) {
      ...
      default:
        if (key === "fundingRequired") return <span className="font-semibold">{deal.fundingRequired}</span>;
  ...
  return (
    <div className="w-full space-y-4 font-sans">
        ...
        {loading ? (
          ...
          <>
            <div className="overflow-auto" style={{ maxHeight: "70vh" }}>
              <style>{`
                .cdd-th { color: #faf7f2 !important; line-height: 1.1; font-size: 0.75rem !important; font-weight: 600 !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; font-family: inherit !important; vertical-align: top !important; }
                .cdd-th-draggable { cursor: grab; }
                .cdd-th-draggable:active { cursor: grabbing; }
                ...
                .cdd-th-label { flex: 1 1 auto; min-width: 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; white-space: normal; overflow-wrap: break-word; line-height: 1.2; text-align: left; }
              ...
              <table className="border-collapse bigt-fit" style={{ tableLayout: "auto" }}>
                <thead>
                  <tr className="bg-[#4a352f]">
                    <th className="cdd-th py-3 px-3 relative border-r border-[#e6d7c3] sticky top-0 left-0 z-30"
                      style={{ backgroundColor: "#4a352f", ...widthStyle("__name__", "180px", "210px") }}>
                      <div className="flex items-start gap-1 min-w-0">
                        <span className="cdd-th-label">Business Name</span>
                    ...
                    {visibleColumns.map((key) => {
                      ...
                      return (
                        <th key={key} draggable={!resizingColumn}
                          ...
                          className={`cdd-th cdd-th-draggable py-3 px-3 relative border-r border-[#e6d7c3] sticky top-0 z-20 select-none transition-opacity ${draggedColumn === key ? "opacity-40" : ""}`}
                          ...
                          <div className="flex items-start gap-1 min-w-0">
                            ...
                            <span className="cdd-th-label">{col.label}</span>
                    ...
                    <th className="cdd-th py-3 px-3 relative text-center whitespace-nowrap sticky top-0 z-20"
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((deal) => (
                      <tr key={deal.rowKey} className="border-b border-[#f0e6d9] transition-all"
                        style={{ backgroundColor: hoveredRow === deal.rowKey ? "#fdf8f4" : undefined }}
                        onMouseEnter={() => setHoveredRow(deal.rowKey)} onMouseLeave={() => setHoveredRow(null)}>
                        <td className={`${ds.cell} ${ds.fontSize} text-[#4a352f] sticky left-0 border-r border-b border-[#e6d7c3] z-10 transition-colors`}
                          style={{ ...widthStyle("__name__", "180px", "210px"), backgroundColor: hoveredRow === deal.rowKey ? "#fdf8f4" : "#ffffff" }}>
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start gap-1.5 flex-wrap">
                                <span className={`${ds.fontSize} leading-snug text-[#4a352f]`}>{deal.name}</span>
                                <button onClick={() => setSelectedDeal(deal)} className="text-[#a89482] hover:text-[#7d5a50] transition-colors flex-shrink-0 mt-0.5"
                                  aria-label={`View profile for ${deal.name}`} title="View profile">
                                  <Eye size={13} />
                                </button>
                              </div>
                            </div>
      ...
      {/* ─── Business profile pop-up ───────────────────────────────────────
          The same modal the pipeline table opens from its business-name eye
          icon, handed the same thing: the mapped row object. Rows here are
          built with the pipeline table's field set for exactly that reason. */}
      {selectedDeal && (
        <SMEDetailsModal
          sme={selectedDeal}
          isOpen={!!selectedDeal}
  ...
  return (
    <div className="w-full font-sans">
      ...
      {/* Both tables stay mounted and the inactive one is hidden, rather than
          unmounted. The tab count badges are fed by the tables themselves, and
          a table that hasn't rendered reports nothing — so an unmounted
          Declined Deals tab would always read 0 until you clicked it. */}
      <div className="bg-white rounded-b-2xl border border-[#e6d7c3] border-t-0 shadow-lg min-h-[500px]">
        ...
        <div className="p-6" style={{ display: activeTab === "declined-deals" ? "block" : "none" }}>
          <DeclinedSupportDealsTable onCountChange={handleDeclinedCount} />
        </div>
      </div>
    </div>
  );
