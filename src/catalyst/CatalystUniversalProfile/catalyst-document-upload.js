...
export default function CatalystDocumentUpload({ data = {}, updateData }) {
    ...
    const loadDocuments = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          const documentsData = {}
          // Populate documentsData based on documentsList and profileData.documentUpload
          ...
          updateData(documentsData) // Sync with parent state
        } else {
          // Initialize with passed data if no Firebase data
          setFormData(data)
        }
      } catch (error) {
        console.error("Error loading documents:", error)
        setFormData(data) // Fallback to local data on error
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleFileChange = async (documentId, files) => {
    ...
    try {
      ...
      await updateData(updatedData) // Update parent state
    } catch (error) {
  ...
  const handleDeleteFile = async (documentId, fileIndex) => {
    ...
    if (fileToDelete && fileToDelete.path) {
      try {
        ...
        console.log("File deleted from storage:", fileToDelete.path)
  ...
  if (isLoading) {
    return (
      <div style={{ padding: "20px", backgroundColor: "#faf8f6", borderRadius: "12px" }}>
        <h2 style={{ fontSize: "24px", fontWeight: "bold", color: "#8b4513", marginBottom: "24px" }}>
          Document Upload
        </h2>
    ...
    <div style={{ padding: "20px", backgroundColor: "#faf8f6", borderRadius: "12px" }}>
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "20px" }}>
          ...
          >
            <h4 style={{ fontSize: "14px", fontWeight: "600", color: "#2e7d32", marginBottom: "12px" }}>
          ...
          >
            <h4 style={{ fontSize: "14px", fontWeight: "600", color: "#f57c00", marginBottom: "12px" }}>
              ⚠️ File Size Limit
            </h4>
            <ul style={{ margin: "0", paddingLeft: "20px", color: "#5d4037", fontSize: "14px", lineHeight: "1.5" }}>
              <li>
          ...
          >
            <h4 style={{ fontSize: "14px", fontWeight: "600", color: "#c62828", marginBottom: "12px" }}>
              🚫 Unsupported Formats
            </h4>
            <ul style={{ margin: "0", paddingLeft: "20px", color: "#5d4037", fontSize: "14px", lineHeight: "1.5" }}>
              <li style={{ marginBottom: "4px" }}>
                No ZIP/RAR folders, executable files (.exe), or Google Docs/Drive links
              </li>
              <li style={{ marginBottom: "4px" }}>
                Please download and upload original files directly (no screenshots or photos of screens)
              </li>
            </ul>
          </div>
        </div>
            ...
            {documentsList.map((document, index) => (
              ...
              >
                ...
                <td style={{ padding: "6px 8px", verticalAlign: "middle" }}>
                  <div style={{ fontWeight: "500", color: "#5d4037", fontSize: "12px", lineHeight: "1.2" }}>
                    {document.label}
                  </div>
                </td>
                <td style={{ padding: "6px 8px", verticalAlign: "middle" }}>
                  <div style={{ color: "#6d4c41", fontSize: "11px", lineHeight: "1.2" }}>{document.description}</div>
                </td>
                <td style={{ padding: "6px 8px", textAlign: "center", verticalAlign: "middle" }}>
