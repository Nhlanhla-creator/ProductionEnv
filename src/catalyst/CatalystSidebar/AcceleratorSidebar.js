...
function AcceleratorSidebar({ companyName }) {
...
const handleLogout = async () => {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        setUser(currentUser)
        try {
          const userDocRef = doc(db, "catalystProfiles", currentUser.uid)
          ...
          if (userDocSnap.exists()) {
            ...
            setUserName(registeredName || "Accelerator Program")
          } else {
            console.log("User document not found!")
            setUserName(currentUser.displayName || currentUser.email.split("@")[0] || "Accelerator Program")
          }
        } catch (error) {
  ...
  const menuItems = [
    { id: "home", label: "Home", icon: <Home size={18} />, route: "/HomePageCatalysts" },
    {
      id: "profile",
      label: "My Profile",
      icon: <User size={18} />,
        ...
        <div className={styles.companyHeader}>
          ...
          {!isSidebarCollapsed && (
            <div className={styles.companyInfo}>
              ...
              <div className={styles.dashboardTitle}>Catalyst Dashboard</div>
            </div>
          )}
        </div>
