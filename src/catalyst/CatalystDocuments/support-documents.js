...
const CatalystDocuments = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        try {
          ...
          setProfileData(data);
          setSubmittedDocuments(checkSubmittedDocs(DOCUMENTS, data));
        } catch (err) {
  ...
  const filteredDocuments = DOCUMENTS.filter((d) => {
    const isSubmitted = submittedDocuments.includes(d);
    const matchFilter =
      filter === "all" ||
      (filter === "submitted" && isSubmitted) ||
      (filter === "pending" && !isSubmitted);
    const matchSearch = d.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return (
      <a
        ...
        style={{
          ...
          color: "#5d4037", textDecoration: "none", fontSize: "12px",
          fontWeight: "500", padding: "4px 0", borderBottom: "1px solid #5d4037",
        }}
      >
        <FileText size={14} />
        View Document
  ...
  const getStatusBadge = (docLabel) => {
    const isUploaded = !!getDocumentURL(docLabel, profileData);
    return (
      <span style={{
  ...
  if (!auth.currentUser && !loading) {
    return (
      <div style={{ textAlign: "center", padding: "80px 32px", backgroundColor: "#f5f2f0", borderRadius: "16px", border: "2px dashed #d7ccc8", color: "#6d4c41", fontSize: "1.125rem", fontWeight: "500" }}>
    ...
    <>
      <style jsx global>{`
        ...
        @media (max-width: 1024px) {
          .documents-container { margin-left: 0 !important; padding-left: 16px !important; padding-right: 16px !important; }
          .sidebar-space { display: none; }
          .document-controls { flex-direction: column; gap: 16px; align-items: stretch; }
          .search-box { width: 100% !important; }
        }
        @media (max-width: 768px) {
          .documents-container { padding: 16px !important; }
          .documents-table-container { overflow-x: auto; }
          .documents-table { min-width: 700px; }
        }
      `}</style>
      ...
      >
        ...
        <div style={{ width: "100%", maxWidth: "1400px", margin: "0 auto" }}>
          ...
          <div className="document-controls" style={{
            ...
            marginBottom: "32px", padding: "20px 24px", backgroundColor: "#f5f2f0",
            borderRadius: "12px", border: "1px solid #d7ccc8", width: "100%", boxSizing: "border-box",
          }}>
              ...
              {["all", "submitted", "pending"].map((type) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px", fontWeight: "500", fontSize: "0.875rem",
                    cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.5px", minWidth: "100px",
                  }}
                >
            ...
            <input
              ...
              style={{
                padding: "12px 16px", border: "2px solid #d7ccc8", borderRadius: "8px",
                fontSize: "0.875rem", backgroundColor: "#faf8f6", color: "#5d4037",
                minWidth: "200px", width: "280px", outline: "none", boxSizing: "border-box",
              }}
            />
          </div>
          ...
          {/* Table */}
          {loading ? (
            <div style={{ textAlign: "center", padding: "80px 32px", backgroundColor: "#f5f2f0", borderRadius: "16px", border: "2px dashed #d7ccc8", color: "#6d4c41" }}>
              Loading documents...
            </div>
            ...
            <div className="documents-table-container" style={{ backgroundColor: "white", borderRadius: "12px", boxShadow: "0 4px 6px rgba(0,0,0,0.1)", overflow: "hidden", border: "1px solid #d7ccc8", overflowX: "auto" }}>
              <table className="documents-table" style={{ width: "100%", borderCollapse: "collapse", minWidth: "800px" }}>
                <thead>
                  <tr style={{ backgroundColor: "#8d6e63", color: "white", height: "50px" }}>
                    {["Document Name", "Uploaded Document", "Last Updated", "Status", "Actions"].map((h) => (
                      <th key={h} style={{ padding: "16px 20px", textAlign: "left", fontWeight: "600", fontSize: "12px", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: "2px solid #6d4c41" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filteredDocuments.map((docLabel, index) => {
                    const base = DOCUMENT_PATHS[docLabel];
                    const parts = base.split(".");
                    const timestampPath = parts.length === 1 ? `${parts[0]}UpdatedAt` : `${parts.slice(0, -1).join(".")}.UpdatedAt`;
                    const updatedAt = get(profileData, timestampPath);

                    return (
                      <tr key={docLabel}
                        style={{ backgroundColor: index % 2 === 0 ? "white" : "#faf8f6", borderBottom: "1px solid #e8d8cf", height: "60px" }}
                        onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#efebe9"}
                        onMouseLeave={(e) => e.currentTarget.style.backgroundColor = index % 2 === 0 ? "white" : "#faf8f6"}
                      >
                        ...
                        <td style={{ padding: "16px 20px", fontSize: "13px", color: "#6d4c41", verticalAlign: "middle" }}>
                        ...
                        <td style={{ padding: "16px 20px", verticalAlign: "middle" }}>
                          <label style={{
                            display: "inline-flex", alignItems: "center", gap: "6px",
                            padding: "8px 16px", backgroundColor: "#a67c52", color: "white",
                          ...
                          >
                            <Upload size={12} />
                            {getDocumentURL(docLabel, profileData) ? "Update" : "Upload"}
                            <input type="file" style={{ display: "none" }} onChange={(e) => handleFileUpload(docLabel, e.target.files[0])} />
                          </label>
                        </td>
                      </tr>
                    );
