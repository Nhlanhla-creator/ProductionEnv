          ...
          >
            ...
            >
              <div>
                ...
                <span style={{ display: "block", color: DARK_BROWN_COLORS.tertiary, fontSize: "14px" }}>Current</span>
              </div>
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (isInvestorView && viewingSMEId) {
        // In investor view, use the SME's ID
  ...
  const handleSaveData = async (data) => {
    ...
    try {
      if (modalSection === "employee-composition") {
        ...
        const employeeQuery = query(collection(db, "employeeComposition"), where("userId", "==", user.uid))
        const employeeSnapshot = await getDocs(employeeQuery)
        ...
        // Check if document exists
        const turnoverQuery = query(collection(db, "turnoverData"), where("userId", "==", user.uid))
        const turnoverSnapshot = await getDocs(turnoverQuery)
