          ...
          {isSelect ? (
            <select
              ...
              onChange={e => {
                ...
                n[category] = [...n[category]]; n[category][idx] = e.target.value
                setData(n)
              }}
              className={inputCls}
            >
...
const BalanceSheetSection = ({ title, data, setData, months, selectedYear }) => {
              ...
              <div key={month}>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    n[key] = [...n[key]]; n[key][idx] = e.target.value
                    setData(n)
                  }}
                  className={inputCls}
                />
...
// ==================== SAVE HANDLERS ====================
const savePnl = async (uid, pnlData, year = new Date().getFullYear()) => {
...
// ==================== MAIN MODAL ====================
const UniversalAddDataModal = ({ isOpen, onClose, currentTab, user, onSave, loading, financialYearStart = "Jan" }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!snapId) return
        // Try year-keyed first, fall back to legacy for migration
  ...
  const renderFields = (data, setData, fields) =>
    fields.map(([category, label, opts]) => (
      <MonthlyInputRow key={category} category={category} label={label} data={data} selectedYear={selectedYear} setData={setData} months={fyMonths} {...(opts || {})} />
    ))
  ...
  const textareaRow = (value, onChange, placeholder) => (
    <div className="mb-5">
      ...
      <textarea value={value} onChange={onChange} placeholder={placeholder} className="w-full p-2.5 rounded border border-[#e8ddd4] min-h-[80px] text-sm resize-y" />
    </div>
  )
  ...
  const inputField = (label, type, value, onChange, placeholder = "", extra = {}) => (
    <div className="mb-4">
      <label className="block text-mediumBrown mb-1 text-xs font-semibold">{label}</label>
      <input type={type} value={value} onChange={onChange} placeholder={placeholder} {...extra} className="w-full p-2.5 rounded border border-[#e8ddd4] text-sm" />
    </div>
  )
                ...
                <div key={idx} className="mt-5 p-4 bg-[#f5f0eb] rounded-lg">
                  ...
                  {custom?.items && Object.keys(custom.items).map(itemKey => (
                    <div key={itemKey} className="mb-4">
                      ...
                      <div className="grid grid-cols-12 gap-1">
                        {months.map((month, idx2) => (
                          <div key={month}>
                            <label className={labelCls}>{month}</label>
                            <input type="number" step="0.01" value={custom.items[itemKey]?.[idx2] || ""}
                              onChange={e => setBalanceSheetData(prev => {
                                const cats = [...prev.assets.customCategories]
                                cats[idx] = { ...cats[idx], items: { ...cats[idx].items, [itemKey]: [...(cats[idx].items[itemKey] || Array(12).fill(""))].map((v, i) => i === idx2 ? e.target.value : v) } }
