...
const CatalystDetailsModal = ({ catalyst, isOpen, onClose }) => {
    ...
    const [catalystDocuments, setCatalystDocuments] = useState({})
    ...
    useEffect(() => {
        if (activeTab === "documents" && catalyst) {
            const catalystId = catalyst.originalCatalystId || catalyst.id?.split("_")[0] || catalyst.id
            fetchCatalystDocuments(catalystId)
        }
    }, [activeTab, catalyst])

    const fetchCatalystDocuments = async (catalystId) => {
        setDocsLoading(true)
        try {
            const docRef = doc(db, "catalystProfiles", catalystId)
            const docSnap = await getDoc(docRef)
            if (docSnap.exists()) {
                const data = docSnap.data()
                const uploadedDocs = data.formData?.documentUpload || {}
                setCatalystDocuments(uploadedDocs)
            }
        } catch (err) {
            console.error("Error fetching catalyst documents:", err)
        } finally {
                                    ...
                                    {programs.map((program, i) => (
                                        <div key={i} style={categoryCardStyle}>
                                            <h4 style={categoryTitleStyle}>
                                                Program {i + 1}: {program.name || "Unnamed"}
                                            </h4>
                                            <div style={infoGridStyle}>
