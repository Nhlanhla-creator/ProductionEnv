// hooks/ESGAnalysisModal.jsx

import { useState, useEffect } from "react";
import { useESGAnalysis } from "./useESGAnalysis";
import { doc, setDoc } from "firebase/firestore";
import { db } from "../../firebaseConfig";
import "./AnalysisModal.css";

const ESGAnalysisModal = ({
  isOpen,
  onClose,
  kpiTitle,
  kpiKey,
  ...
  const {
    ...
    generateAnalysis,
    clearAnalysis,
  } = useESGAnalysis(currentUser, section, subSection);
  ...
  const handleSaveToNotes = async () => {
    ...
    try {
      const savedAnalysisRef = doc(
        db, 
        `users/${currentUser.uid}/savedESGAnalyses`, 
        `${section}_${subSection}_${kpiKey}_${Date.now()}`
      );
  ...
  const formatValue = (val) => {
    ...
    if (typeof val === 'number') {
      ...
      if (kpiKey.includes("Percentage") || kpiKey.includes("Rate") || kpiKey.includes("Level")) {
      ...
      <div className="analysis-modal-container" onClick={(e) => e.stopPropagation()}>
        <div className="analysis-modal-header">
          <div className="analysis-modal-title">
            <span className="ai-badge">🌿 AI ESG Analysis</span>
            <h2>{kpiTitle}</h2>
          </div>
...
export default ESGAnalysisModal;