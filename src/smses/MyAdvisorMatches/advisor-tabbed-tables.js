...
const STATUS_COLORS = {
  ...
  "Engaged/Placed": { color: "#E8F5E8", textColor: "#388E3C" },
}
...
const SuccessfulAdvisorDealsTable = ({ deals = [], loading, onCountChange, onNotify }) => {
  ...
  const stageOptions = useMemo(() => uniqueOf((d) => d.smeStage), [uniqueOf])
  ...
  const filteredDeals = useMemo(() => {
    ...
    const rows = deals.filter((d) => {
      ...
      if (!matchesAny(f.smeStage, d.smeStage)) return false
      ...
      const rating = Number(d.performanceRating) || 0
    ...
    if (sortConfig?.key) {
      const accessors = {
        ...
        status: (r) => r.status,
        rating: (r) => Number(r.performanceRating) || 0,
        matchPercentage: (r) => r.matchPercentage || 0,
        smeStage: (r) => r.smeStage,
        contactedAt: (r) => toDateSafe(r.contactedAt)?.getTime() ?? 0,
      }
  ...
  const activeFilterCount =
    (f.name.trim() ? 1 : 0) +
    f.engagementModel.length +
    f.sectorExperience.length +
  ...
  const renderCell = (key, d, rowBg) => {
    ...
    switch (key) {
      ...
      case "contactedAt":
        return (
          <td key={key} style={style}>
            {formatDateValue(d.contactedAt) || <span className="text-[#a89482]">-</span>}
          </td>
        )
                ...
                {orderedColumns.map((key) => {
                  ...
                  return (
                    <th
                      ...
                      className={`ad-th ad-th-draggable font-semibold uppercase tracking-wider text-xs top-0 select-none transition-opacity ${
                    ...
                    >
                      <GripVertical size={11} className="ad-th-grip" />
                      <div className={`ad-th-row ${col.align === "center" ? "justify-center" : ""}`}>
                        <span className="ad-th-label" title={col.label}>
                          {col.label}
                        </span>
                        <span className="ad-th-tools">
                ...
                filteredDeals.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          <span className="font-medium text-[#4a352f] break-words text-sm">{d.advisorName}</span>
                          <button
                        ...
                        {d.sectorExperience && d.sectorExperience !== "-" && (
                          <div className="text-[10px] text-[#a89482] mt-0.5">{d.sectorExperience}</div>
                        )}
                      </td>
      ...
      {headerFilterOpen && (
        <PopupPortal>
          ...
          >
            ...
            {[
              { type: "engagementModel", label: "Engagement Model", options: engagementOptions },
              { type: "sectorExperience", label: "Sector", options: sectorOptions },
              { type: "status", label: "Status", options: statusOptions },
              { type: "smeStage", label: "Business Stage", options: stageOptions },
            ].map(
              ({ type, label, options }) =>
                headerFilterOpen.type === type && (
                  <div key={type}>
            ...
            >
              ...
              <div className="p-6">
                <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
                  {[
                    ["Engagement model", selectedDeal.engagementModel],
                    ["Sector", selectedDeal.sectorExperience],
                    ["Business stage", selectedDeal.smeStage],
                    ["Start date", formatDateValue(selectedDeal.startDate) || "-"],
                    ["Date contacted", formatDateValue(selectedDeal.contactedAt) || "-"],
                    ["Deal value", selectedDeal.dealAmount],
                    ["Match at connection", `${selectedDeal.matchPercentage || 0}%`],
                ...
                <div className="bg-[#faf7f2] border border-[#e6d7c3] rounded-lg p-4 mt-4">
                  ...
                  <p className="text-sm text-[#7d5a50] m-0 leading-relaxed">{selectedDeal.servicesDelivered || "-"}</p>
                </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   Tab shell
   ════════════════════════════════════════════════════════════════════════ */
const AdvisorTabbedTables = ({ filters, stageFilter, onConnectionRequested }) => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const rows = snapshot.docs
          ...
          .filter((r) => ENGAGED_STATUSES.includes(normalizeAdvisorStatus(r.status)))
          .map((r) => ({
            id: r.id,
            advisorId: r.advisorId,
            advisorName: r.advisorName || "Advisor",
            engagementModel: r.advisorEngagementModel || r.engagementModel || "-",
            sectorExperience: r.advisorSector || "-",
            smeStage: r.smeStage || "-",
            startDate: r.engagementStartAt || r.acceptedAt || r.createdAt || null,
            contactedAt: r.smeActedAt || r.createdAt || null,
            dealAmount: r.dealAmount || r.contractValue || "-",
            status: normalizeAdvisorStatus(r.status),
            performanceRating: r.performanceRating ?? 0,
            matchPercentage: r.matchPercentage ?? 0,
            servicesDelivered: r.servicesDelivered || "Advisory services",
          }))
        setEngagements(rows)
        setLoadingDeals(false)
      },
