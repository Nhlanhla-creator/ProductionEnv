...
import SupplierTable, { SME_SUPPLIER_COLLECTION, normalizeSupplierStatus } from "./supplier-table"
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   The header scrolled away while the pinned body cells stayed frozen, and its
   narrow default widths left each label fighting a grip, a sort control and a
   filter control for ~60px of chrome — so the browser broke labels mid-word
   ("PROD UCT / SERV..."). This table now owns its head, toolbar, filters and
   row actions, exactly like customer-tabbed-tables.jsx.

   SupplierTable (the matches tab) still uses the kit and can share a page with
   this table, so every selector here is prefixed st- (supplier table) and the
   sticky headers declare `position: sticky !important` to survive the kit's
   global <th> rule.
   ════════════════════════════════════════════════════════════════════════ */

const ENGAGED_STATUSES = ["Accepted"]

/* One collection, keyed by supplierId, written and read in the same shape.
   The old flow wrote reviews keyed on supplierId but the Rating column read
   `performanceRating` off the SME engagement doc, which nothing ever updated —
   so a review a customer left never moved the star count here. */
const REVIEWS_COLLECTION = "supplierReviews"
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   Reads the live reviews handed down on the deal rather than re-fetching, so
   a submitted review updates the Rating column and this list at once.
   ════════════════════════════════════════════════════════════════════════ */
const ReviewsModal = ({ deal, isOpen, onClose, onNotify }) => {
  ...
  const average = deal.performanceRating || 0

  const submit = async () => {
    if (rating === 0) {
      setError("Choose a star rating before you submit.")
      return
    }
    ...
    try {
      ...
      const registeredName = profileSnap.exists()
        ? profileSnap.data().entityOverview?.registeredName || user.displayName || "A customer"
      ...
      await addDoc(collection(db, REVIEWS_COLLECTION), {
        ...
        customerId: anonymous ? null : user.uid,
        customerName: anonymous ? "Anonymous" : registeredName,
        engagementId: deal.id,
        date: new Date().toISOString().slice(0, 10),
        createdAt: serverTimestamp(),
      })

      onNotify?.("success", "Review posted")
      onClose()
    } catch (err) {
      console.error("Error submitting review:", err)
      setError("The review didn't save. Try again.")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-white rounded-2xl max-w-[620px] w-full max-h-[85vh] overflow-y-auto shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="bg-gradient-to-br from-[#4a352f] to-[#7d5a50] p-4 text-white">
          <div className="flex items-center justify-between">
            <div className="min-w-0">
              <p className="text-xs font-semibold text-[#f5f0e1] uppercase tracking-wider">Reviews</p>
        ...
        <div className="p-6 space-y-5">
          <div className="bg-[#faf7f2] border border-[#e6d7c3] rounded-xl p-5 text-center">
            <div className="flex items-center justify-center gap-3">
              <StarRating rating={average} size={22} />
              <span className="text-xl font-bold text-[#4a352f]">{average.toFixed(1)}/5</span>
            </div>
            <p className="text-xs text-[#a89482] mt-2 mb-0">
              {reviews.length === 0
                ...
                <div key={review.id} className="bg-[#faf7f2] border border-[#e6d7c3] rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-[#4a352f]">{review.customerName || "A customer"}</span>
                    </div>
                    <span className="text-[10px] text-[#a89482]">{formatDateValue(review.date) || ""}</span>
                  </div>
                  {review.comment && <p className="text-xs text-[#7d5a50] m-0 leading-relaxed">{review.comment}</p>}
                </div>
              ))}
            </div>
          )}

          <div className="bg-[#f5f0e1] border border-[#e6d7c3] rounded-xl p-5">
            <h4 className="text-sm font-semibold text-[#4a352f] m-0 mb-3">Rate this supplier</h4>
            <StarRating rating={rating} size={26} readOnly={false} onRatingChange={setRating} />

            <label className="block text-xs font-semibold text-[#4a352f] mt-4 mb-1.5">
              <MessageSquare size={12} className="inline mr-1.5 align-middle" />
              Comment (optional)
            </label>
            <textarea
              value={comment}
              ...
              rows={3}
              maxLength={500}
              placeholder="What was working with this supplier like?"
              className="w-full px-3 py-2 border border-[#c8b6a6] rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#7d5a50]/20"
            />
            <div className="text-[11px] text-[#a89482] text-right mt-1">{comment.length}/500</div>

            <label className="flex items-center gap-2 mt-2 text-xs text-[#7d5a50]">
              <input
                type="checkbox"
                checked={anonymous}
            ...
            {error && <p className="text-xs text-[#D32F2F] mt-3 mb-0">{error}</p>}
          </div>
        </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   Successful supplier deals — column configuration.

   Supplier Name is the pinned first column and Action the last, so neither
...
const COLUMN_DEFS = {
  productService: {
    label: "Product / Service", width: 210, filterType: "productService", visible: true, priority: 2, sortable: true,
    tooltip: "What this supplier was appointed to deliver, taken from the request you raised.",
  },
  dealAmount: {
    label: "Deal Value", width: 152, filterType: "dealAmount", visible: true, priority: 2, sortable: true,
    tooltip: "What the engagement is worth. Sorting uses the underlying number, not the formatted label.",
  },
  startDate: {
    label: "Start Date", width: 152, filterType: "startDate", visible: true, priority: 3, sortable: true,
    tooltip: "When the engagement started, or when you accepted the supplier if no start date was captured.",
  },
  status: {
    label: "Status", width: 148, filterType: "status", visible: true, priority: 1, sortable: true,
    tooltip: "Where the engagement stands — accepted, under active contract, or completed.",
  },
  rating: {
    label: "Rating", align: "center", width: 158, filterType: "rating", visible: true, priority: 1, sortable: true,
    tooltip: "The average star rating across every review left on this supplier, with the review count in brackets. Click the star in Action to add yours.",
  },

  location: {
    label: "Location", width: 152, filterType: "location", visible: false, priority: 4, sortable: true,
    tooltip: "Where the supplier is based. Also shown under the supplier name.",
  },
  matchPercentage: {
    label: "Match %", align: "center", width: 138, filterType: "matchPercentage", visible: false, priority: 4, sortable: true,
    tooltip: "How well this supplier scored against your request at the moment you appointed them. It's a snapshot, not a live score.",
  },
  bbbeeLevel: {
    label: "B-BBEE Level", width: 162, filterType: "bbbeeLevel", visible: false, priority: 4, sortable: true,
    tooltip: "The supplier's B-BBEE contributor level, which feeds your own procurement recognition.",
  },
  requestedAt: {
    label: "Date Requested", width: 164, filterType: "requestedAt", visible: false, priority: 4, sortable: true,
    tooltip: "When you first raised the quote request that led to this engagement.",
  },
}
...
/* Supplier Name and Action can't be hidden or reordered, so they aren't in
...
const FIXED_WIDTHS = { [NAME_KEY]: 226, [ACTION_KEY]: 200 }
...
const NAME_TOOLTIP = "The supplier you appointed, with their location underneath. Click the eye to open the full engagement record."
const ACTION_TOOLTIP = "Open the engagement, leave a review, or use quick actions for more."
...
const VIEWS_STORAGE_KEY = "supplier-deals-views-v3"
const FILTERS_STORAGE_KEY = "supplier-deals-filters-v1"
...
/* ════════════════════════════════════════════════════════════════════════════
   Successful supplier deals table
   ════════════════════════════════════════════════════════════════════════ */
const SuccessfulSupplierDealsTable = ({ deals = [], loading, onCountChange, onNotify }) => {
  ...
  /* One live listener for all reviews, keyed by supplierId. */
  useEffect(() => {
  ...
  const dealsWithRatings = useMemo(
    () =>
      deals.map((deal) => {
        ...
        const average = reviews.length > 0 ? reviews.reduce((s, r) => s + (Number(r.rating) || 0), 0) / reviews.length : 0
        return { ...deal, reviews, performanceRating: Number.parseFloat(average.toFixed(1)) }
      }),
    [deals, reviewsBySupplier],
  )
  ...
  const bbbeeOptions = useMemo(() => uniqueOf((d) => d.bbbeeLevel), [uniqueOf])
  const statusOptions = useMemo(() => {
    const found = uniqueOf((d) => d.status)
    return found.length > 0 ? found : ENGAGED_STATUSES
  }, [uniqueOf])
  ...
  const filteredDeals = useMemo(() => {
    ...
    const rows = dealsWithRatings.filter((d) => {
      ...
      if (!includesText(f.productService, d.productService)) return false
      ...
      if (!matchesAny(f.bbbeeLevel, d.bbbeeLevel)) return false
    ...
    if (sortConfig?.key) {
      const accessors = {
        ...
        requestedAt: (r) => toDateSafe(r.requestedAt)?.getTime() ?? 0,
      }
  ...
  /* Keep the open modals pointed at live rows so a new review shows up
     immediately in whichever panel is open. */
  const liveSelectedDeal = useMemo(
    () => (selectedDeal ? dealsWithRatings.find((d) => d.id === selectedDeal.id) || selectedDeal : null),
    [selectedDeal, dealsWithRatings],
  )
  const liveReviewDeal = useMemo(
    () => (reviewDeal ? dealsWithRatings.find((d) => d.id === reviewDeal.id) || reviewDeal : null),
    [reviewDeal, dealsWithRatings],
  )
  ...
  const renderCell = (key, d, rowBg) => {
    ...
    switch (key) {
      ...
      case "requestedAt":
        return (
          <td key={key} style={style}>
            {formatDateValue(d.requestedAt) || <span className="text-[#a89482]">-</span>}
          </td>
        )
          ...
          <td key={key} style={{ ...style, textAlign: "center" }}>
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="text-[11px] font-semibold text-[#7d5a50]">
                {d.reviews.length > 0 ? `${d.performanceRating}/5 (${d.reviews.length})` : "No reviews"}
              </span>
            </div>
          </td>
        )
      ...
      case "status": {
        const s = dealStatusStyle(d.status)
        return (
          <td key={key} style={style}>
            <span
  ...
  if (loading || (loadingReviews && deals.length > 0)) {
                  ...
                  <PopupPortal>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 py-1.5 px-2 rounded-lg opacity-75">
                        ...
                        <span className="text-sm text-[#4a352f] flex-1">Supplier Name</span>
        ...
        <div className="overflow-auto" style={{ maxHeight: "70vh" }}>
          <style>{`
            /* No 'position: relative' here — that is what the shared kit had,
               ...
               Prefix is st- (supplier table) to avoid colliding with the other
               match tables' styles when they share a page. */
            .st-th { position: sticky !important; color: #faf7f2 !important; vertical-align: top !important; }
            ...
            /* overflow-wrap: normal stops the browser splitting inside a word,
               which is what turned "Deal Value" into "DEAL VAL.." and
          ...
          >
            <thead>
              <tr>
                {/* Supplier Name — pinned first column, resizable like the rest */}
                <th
                  className="st-th font-semibold uppercase tracking-wider text-xs top-0 left-0 z-30 text-left"
                  style={{
                ...
                >
                  <div className="st-th-row">
                    <span className="st-th-label" title="Supplier Name">
                      Supplier Name
                    </span>
                ...
                {orderedColumns.map((key) => {
                  ...
                  return (
                    <th
                      ...
                      className={`st-th st-th-draggable font-semibold uppercase tracking-wider text-xs top-0 select-none transition-opacity ${
                ...
                <th
                  className="st-th text-center font-semibold uppercase tracking-wider text-xs top-0 z-20"
                  style={{
                    backgroundColor: "#4a352f",
                    width: actionWidth,
                    padding: headerPadding,
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-xs text-[#a89482] m-0">
                        {deals.length === 0
                          ? "Appoint a supplier from your matches and the deal will appear here."
                    ...
                    >
                      ...
                      >
                        ...
                        {d.location && d.location !== "-" && (
                          <div className="text-[10px] text-[#a89482] mt-0.5">{d.location}</div>
                        )}
                      </td>
                      ...
                      >
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            onClick={() => setReviewDeal(d)}
                            title="Review this supplier"
                            aria-label="Review this supplier"
          ...
          >
            ...
            <button
              onClick={() => {
                const target = activePopup.row
                closePopup()
                setReviewDeal(target)
              }}
              className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-[#4a352f] hover:bg-[#faf7f2] text-left"
            >
              <Star size={12} /> Review this supplier
            </button>
            <button
              onClick={() => {
                closePopup()
                onNotify?.("info", '"Message supplier" is not wired up yet.')
              }}
              className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-[#4a352f] hover:bg-[#faf7f2] text-left"
            >
        ...
        <PopupPortal>
          ...
          >
            ...
            {[
              ...
              { type: "productService", label: "Product / Service", placeholder: "Search offering..." },
              { type: "dealAmount", label: "Deal Value", placeholder: "Search value..." },
            ].map(
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="min-w-0 flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <h3 className="text-sm font-bold mt-0.5 truncate">{liveSelectedDeal.supplierName}</h3>
                    </div>
                  </div>
              ...
              <div className="p-6">
                <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
                  {[
                    ["Product / service", liveSelectedDeal.productService],
                    ["Deal value", liveSelectedDeal.dealAmount],
                    ["Location", liveSelectedDeal.location],
                    ["B-BBEE level", liveSelectedDeal.bbbeeLevel],
                    ["Start date", formatDateValue(liveSelectedDeal.startDate) || "-"],
                    ["Quote requested", formatDateValue(liveSelectedDeal.requestedAt) || "-"],
                    ["Match at appointment", `${liveSelectedDeal.matchPercentage || 0}%`],
                ...
                <div className="bg-[#faf7f2] border border-[#e6d7c3] rounded-lg p-4 mt-4">
                  <div className="text-[10px] uppercase tracking-wide text-[#a89482] font-semibold mb-2">
                    Reviews ({liveSelectedDeal.reviews.length})
                  </div>
                  {liveSelectedDeal.reviews.length > 0 ? (
                    <>
                      <div className="flex items-center gap-3 mb-3">
                        <StarRating rating={liveSelectedDeal.performanceRating} size={18} />
                        <span className="text-base font-bold text-[#7d5a50]">{liveSelectedDeal.performanceRating}/5</span>
                      </div>
                      <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto">
                        {liveSelectedDeal.reviews.map((r) => (
                          <div key={r.id} className="bg-white border border-[#e6d7c3] rounded p-2.5">
                            <div className="flex items-center gap-2 mb-1">
                              <StarRating rating={r.rating} size={12} />
                              <span className="text-[11px] font-semibold text-[#4a352f]">{r.customerName || "A customer"}</span>
                    ...
                    <p className="text-sm text-[#a89482] m-0">No reviews yet.</p>
                  )}
                </div>
              </div>
...
const SupplierTabbedTables = ({
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const rows = snapshot.docs
          ...
          .filter((r) => r.status && ENGAGED_STATUSES.includes(normalizeSupplierStatus(r.status)))
          .map((r) => ({
            id: r.id,
            supplierId: r.supplierId,
            supplierName: r.supplierName || "Supplier",
            productService: r.originalRequest?.serviceRequested || r.supplierOffering || "-",
            location: r.supplierLocation || "-",
            bbbeeLevel: r.supplierBbbeeLevel || "-",
            dealAmount: r.dealAmount || r.contractValue || "-",
            startDate: r.engagementStartAt || r.acceptedAt || r.createdAt || null,
            requestedAt: r.smeActedAt || r.createdAt || null,
            status: normalizeSupplierStatus(r.status),
      ...
      (error) => {
        console.error("Supplier deals listener failed:", error)
        setLoadingDeals(false)
      },
    )
  ...
  const TABS = [
    { id: "my-matches", label: "Supplier Matches", icon: Users, count: matchesCount },
