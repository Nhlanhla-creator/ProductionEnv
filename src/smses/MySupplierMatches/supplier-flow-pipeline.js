...
import {
  SME_SUPPLIER_COLLECTION,
  normalizeSupplierStatus,
  SUPPLIER_STAGE_FILTER_EVENT,
  SUPPLIER_ROWS_EVENT,
  SUPPLIER_ROWS_REQUEST_EVENT,
} from "./supplier-table"
...
const STAGE_DEFINITIONS = [
  { id: "new_match", name: "New Match", statuses: ["New Match"], icon: Target, tooltip: "Suppliers matched to your request that you haven't acted on yet." },
  { id: "viewed", name: "Viewed", statuses: ["Viewed"], icon: Eye, tooltip: "Profiles you've opened but not shortlisted." },
  { id: "shortlisted", name: "Shortlisted", statuses: ["Shortlisted"], icon: ListChecks, tooltip: "Suppliers you've flagged as worth approaching." },
  { id: "contacted", name: "Contacted", statuses: ["Contacted"], icon: Send, tooltip: "First contact made, no quote requested yet." },
  { id: "quote_requested", name: "Quote Requested", statuses: ["Quote Requested"], icon: FileText, tooltip: "You've asked for a quote and are waiting on the supplier." },
  { id: "quote_received", name: "Quote Received", statuses: ["Quote Received"], icon: FileCheck, tooltip: "A quote or proposal has come back to you." },
  { id: "under_review", name: "Under Review", statuses: ["Under Review"], icon: Search, tooltip: "You're comparing quotes or negotiating terms." },
  { id: "accepted", name: "Accepted", statuses: ["Accepted"], icon: CheckCircle, tooltip: "Supplier appointed and the engagement is live." },
  { id: "declined", name: "Declined", statuses: ["Declined"], icon: XCircle, terminal: true, tooltip: "Supplier declined, or you chose someone else." },
  { id: "closed", name: "Closed", statuses: ["Closed"], icon: Archive, terminal: true, tooltip: "Conversation ended without an appointment." },
]
...
/* Cards are wider than they were and stage names wrap onto another line rather
   ...
   `items-stretch` on the row plus `h-full` on the card body), so "Quote
   Requested" running to two lines never leaves its neighbours sitting short. */
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   Props (all optional):
     totalSuppliers  fallback only. Used when the counts come from Firestore
                     rather than from the table, because "New Match" means
                     "no record exists yet" and there is nothing to count for
                     those rows.
     applications    pre-fetched SmeSupplierApplications rows.
     onStageClick    called with the status name on press, null when cleared.

   Where the counts come from, in order of preference:
     1. SupplierTable's broadcast. Its rows carry a resolved status, so the
   ...
   ════════════════════════════════════════════════════════════════════════ */
export function SupplierFlowPipeline({ totalSuppliers = 0, applications: applicationsProp, onStageClick, className = "" }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(SUPPLIER_ROWS_EVENT, onRows)
    window.dispatchEvent(new Event(SUPPLIER_ROWS_REQUEST_EVENT))
    return () => window.removeEventListener(SUPPLIER_ROWS_EVENT, onRows)
  }, [])
  ...
  const counts = useMemo(() => {
    ...
    if (usingTable) {
      ...
      tableRows.forEach((row) => {
        const status = normalizeSupplierStatus(row.status)
      ...
      result[ENTRY_STAGE_ID] += Math.max(0, (totalSuppliers || 0) - statused)
    }

    if (unrecognised.length > 0) {
      /* Counted under New Match rather than dropped, so the percentages still
         reach 100 and no supplier disappears from the funnel. The console is
         the right place for this — an SME shouldn't be reading about
         STAGE_DEFINITIONS. */
      console.warn(
        "[SupplierFlowPipeline] counted under New Match — no card covers these statuses:",
        [...new Set(unrecognised)],
      )
    }

    return result
  }, [usingTable, tableRows, fetchedRows, totalSuppliers])
  ...
  const handleStageClick = useCallback(
    (status) => {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent(SUPPLIER_STAGE_FILTER_EVENT, { detail: next }))
      }
    },
