...
const debugAiResponse = (text) => {
  ...
  sections.forEach((section, index) => {
    const scorePattern = new RegExp(`###\\s+${index + 1}\\.\\s*${section}[\\s\\S]*?\\*\\*Score:\\*\\*\\s*(\\d)`, "i");
    const scoreMatch = text.match(scorePattern);
    console.log(`🎯 Section ${index + 1} (${section}): Score = ${scoreMatch ? scoreMatch[1] : 'NOT FOUND'}`);
  });
}
...
useEffect(() => {
  if (!auth?.currentUser?.uid || !apiKey || isEvaluating || isReevaluating) return;
      ...
      try {
        ...
        if (aiSnap.exists()) {
          ...
          if (saved.result && saved.includedFundingData === false) {
            console.log("🔄 AUTOMATIC: Funding data now available but previous evaluation doesn't include it");
            console.log("🔄 AUTOMATIC: Running new evaluation with Business Plan, Pitch Deck, and Credit Report");
 ...
 useEffect(() => {
  ...
  }, [profileData, aiEvaluationResult, onScoreUpdate, businessPlanAnalysis, pitchDeckAnalysis, creditReportAnalysis, hasAppliedForFunding, isFundingDataLoaded])
...
const parseAiEvaluationScores = (text) => {
  ...
  Object.entries(allCategories).forEach(([key, labels]) => {
    ...
    for (const label of labels) {
      ...
      for (const pattern of patterns) {
        ...
        if (match) {
          ...
          console.log(`✅ Found score for ${key} (${label}): ${foundScore} using pattern`)
          break
        }
      }
    ...
    if (foundScore === 0 && hasAppliedForFunding && 
        ['businessPlanAnalysis', 'pitchDeckScore', 'creditReport'].includes(key)) {
      console.log(`⚠️ Funding category ${key} score not found - section may be missing from AI response`);
    }
...
const debugFullAiResponse = (text) => {
  ...
  sections.forEach((section, index) => {
    ...
    console.log(`🎯 Section ${index + 1}: "${section.title}" - Score: ${scoreMatch ? scoreMatch[1] : 'NOT FOUND'}`);
  });
  ...
  fundingTerms.forEach(term => {
    ...
    console.log(`🔍 "${term}" mentioned in response: ${found ? 'YES' : 'NO'}`);
  });
}
...
const categoryNames = {
  ...
  const breakdown = Object.entries(categoryMappings).map(([key, label], i) => {
    ...
    if (hasAppliedForFunding && ['businessPlanAnalysis', 'pitchDeckScore', 'creditReport'].includes(key)) {
...
useEffect(() => {
...
}, [hasAppliedForFunding, businessPlanAnalysis, pitchDeckAnalysis, creditReportAnalysis, aiEvaluationResult, profileData])
 ...
 const prepareDataForEvaluation = async (data) => {
  ...
  evaluationData += `Strategy for local value: ${data?.socialImpact?.localValueStrategy || "Not specified"}\n`
   ...
   if (hasAppliedForFunding && isFundingDataLoaded) {
    ...
    console.log("📤 Sending to AI - Business Plan:", businessPlanAnalysis?.content?.substring(0, 100) + "...");
    console.log("📤 Sending to AI - Pitch Deck:", pitchDeckAnalysis?.content?.substring(0, 100) + "...");
    console.log("📤 Sending to AI - Credit Report:", creditReportAnalysis?.content?.substring(0, 100) + "...");
   }
    if (hasAppliedForFunding && isFundingDataLoaded) {
      console.log("✅ Adding funding application sections to evaluation");
      ...
      if (pitchDeckAnalysis?.content) {
        evaluationData += `\n--- PITCH DECK ANALYSIS ---\n`
        evaluationData += `Previous AI Score: ${pitchDeckAnalysis.score || pitchDeckAnalysis.operationalScore}/100\n`
    ...
    console.log("📝 Final evaluation data preview:", evaluationData.substring(0, 500));
 ...
 const runAiEvaluation = async (userId) => {
    ...
    if (hasAppliedForFunding && !isFundingDataLoaded) {
      console.log("⏳ User has applied for funding but data not loaded. Waiting...");
    ...
    try {
      ...
      console.log("📊 Evaluation data prepared, length:", evaluationData.length);
...
**Final Analysis:** [Brief overall assessment]`
 ...
 const refreshAiEvaluation = async () => {
  ...
  try {
    ...
    if (hasAppliedForFunding) {
      console.log("Ensuring funding application data is loaded before evaluation...")
      ...
      <div style={{
        ...
        >
          ...
          <ChevronDown
            size={20}
            style={{
              transform: showFundingSections ? "rotate(180deg)" : "rotate(0deg)",
          ...
          <div style={{
            ...
            {businessPlanAnalysis?.content && (
              <div style={{ marginBottom: "15px" }}>
                <h5 style={{ color: "#5d4037", marginBottom: "8px" }}>Business Plan Analysis</h5>
                <div style={{
            ...
            {pitchDeckAnalysis?.content && (
              <div style={{ marginBottom: "15px" }}>
                <h5 style={{ color: "#5d4037", marginBottom: "8px" }}>Pitch Deck Analysis</h5>
                <div style={{
                  backgroundColor: "white",
                  padding: "12px",
                  borderRadius: "6px",
            ...
            {creditReportAnalysis?.content && (
              <div style={{ marginBottom: "15px" }}>
                <h5 style={{ color: "#5d4037", marginBottom: "8px" }}>Credit Report Analysis</h5>
                <div style={{
                  backgroundColor: "white",
                  padding: "12px",
                  borderRadius: "6px",
    ...
    if (docSnap.exists()) {
      ...
      if (data.triggerFundabilityEvaluation === true && !isEvaluating) {
        ...
        if (!fundingCheckComplete) {
          ...
          while (!fundingCheckComplete && (Date.now() - startTime) < maxWaitTime) {
            await new Promise(resolve => setTimeout(resolve, 500));
          }
        ...
        if (hasAppliedForFunding && !isFundingDataLoaded) {
          console.log("⏳ User has applied for funding, waiting for funding data...");
          setEvaluationError("Loading funding application data...");
          
          const maxWaitTime = 15000;
          const startTime = Date.now();
          
          while (hasAppliedForFunding && !isFundingDataLoaded && (Date.now() - startTime) < maxWaitTime) {
            await new Promise(resolve => setTimeout(resolve, 500));
          }
        ...
        if (saved.result) {
          ...
          if (shouldLoadSavedResult) {
            ...
            if (hasAppliedForFunding && saved.includedFundingData === false) {
              console.log("⚠️ Saved result doesn't include funding data, but automatic re-evaluation will handle this");
            }
          }
        }
      }
...
{isReevaluating && (
  <p style={{ color: "#ff9800", fontSize: "14px", textAlign: "center", marginTop: "10px" }}>
    <RefreshCw size={16} className="spin" style={{ marginRight: "6px" }} />
                ...
                {showAboutScore && (
                  ...
                  >
                    <p style={{ marginBottom: "16px", lineHeight: "1.6" }}>
                      ...
                      evaluates key factors that influence funding decisions across critical areas that determine
...
>
  <p style={{ fontWeight: "bold", marginBottom: "8px", color: "#6d4c41" }}>
    {hasAppliedForFunding ? "Seven key assessment areas:" : "Four key assessment areas:"}
  </p>
  <ul style={{ margin: "0", paddingLeft: "20px", color: "#5d4037" }}>
    <li style={{ marginBottom: "6px" }}>
      <strong>Financial readiness:</strong> Accounting systems, compliance, and up-to-date financial records
    </li>
    <li style={{ marginBottom: "6px" }}>
      <strong>Financial strength:</strong> Revenue growth, profitability, and audited financials
    </li>
    <li style={{ marginBottom: "6px" }}>
      <strong>Operational strength:</strong> Business processes, infrastructure, and operational maturity
    </li>
    <li style={{ marginBottom: "6px" }}>
      <strong>Impact proof:</strong> Job creation, HDG inclusion, environmental responsibility, and CSR investment
    </li>
    {hasAppliedForFunding && (
      <>
        <li style={{ marginBottom: "6px" }}>
          <strong>Business Plan:</strong> Comprehensive business strategy, market analysis, and financial projections
        </li>
        <li style={{ marginBottom: "6px" }}>
          <strong>Pitch Deck:</strong> Investor presentation quality, storytelling, and value proposition clarity
        </li>
        <li style={{ marginBottom: "6px" }}>
          <strong>Credit Report:</strong> Creditworthiness assessment and financial reliability
        </li>
      </>
    )}
  </ul>
  ...
  <p style={{ margin: "0", color: "#5d4037" }}>
    ...
    heavily on operational strength and pitch quality, while mature companies are assessed primarily
    on financial performance.{hasAppliedForFunding && " When you apply for funding, additional weight is given to your Business Plan, Pitch Deck, and Credit Report to provide a comprehensive assessment of your investment readiness."}
    Financial readiness and strength typically carry the highest weights across all stages.
  </p>
                    </div>
                    ...
                    <p style={{ marginBottom: "0", lineHeight: "1.6", fontStyle: "italic", color: "#6d4c41" }}>
                      ...
                      secure financing based on these critical assessment areas.
                    </p>
                  </div>
                )}
              </div>
