...
export function InternInsightsPage() {
  ...
  const processInternsOverTime = () => {
    ...
    // Count applications by month using createdAt from internProfiles
    ...
    // Convert to percentage format for the chart
  ...
  const processInternshipTypeBreakdown = () => {
    ...
    internApplicationsData.forEach(application => {
      // Check if application has internshipRequest with locationFlexibility
  ...
  const processDepartmentalInternDemand = () => {
    ...
    internApplicationsData.forEach(application => {
      // Check if application has internshipRequest with internRoles array
    ...
    internApplicationsData.forEach(application => {
      ...
      if (application.status) {
        ...
        if (typeof application.status === 'object') {
          // If status is an object, try to get the value property
  ...
  // Process rating distribution of all interns - FIXED TO SHOW ALL RATINGS
  const processRatingDistributionOfInterns = () => {
    ...
    // Calculate percentages and ensure all ratings show up with minimum values
    ...
    if (totalRatings === 0) {
      ...
      return {
        ...
        2: 15.0,  // 15% for rating 2
        3: 25.0,  // 25% for rating 3
        4: 35.0,  // 35% for rating 4
        5: 15.0   // 15% for rating 5
      };
    }
    
    // Calculate actual percentages but ensure minimum visibility
    for (let i = 0; i <= 5; i++) {
      const percentage = (ratingDistribution[i] / totalRatings) * 100;
      // Use actual percentage if > 0, otherwise use small value for visibility
  ...
  // FIXED: Calculate average response time by status for ONLY Accepted and Contacted/Interview
  const calculateAverageResponseTimeByStatus = () => {
    ...
    internApplicationsData.forEach(application => {
      // Check if application has both submittedAt and updatedAt dates
      if (application.submittedAt && application.updatedAt) {
        // Get the status first and check if it's one we care about
        let statusValue = application.status;
        if (typeof application.status === 'object') {
          statusValue = application.status.value || application.status;
        }
        ...
        // ONLY process if status is "accepted" or "contacted/interview"
  ...
  useEffect(() => {
    if (loading) return // Don't render charts while loading data
      ...
      createChart(chartRefs.averageResponseTimeByStatus, {
        ...
        options: {
          ...
          scales: {
            x: {
              beginAtZero: true,
              max: 50, // Fixed max at 50 days as requested
              title: {
                display: true,
              ...
              ticks: {
                color: brownPalette.primary,
                font: { size: 10 },
                stepSize: 10, // Count in 10s as requested
    ...
    if (activeTab === "intern-performance") {
      ...
      // Create completely separate arrays to force all labels to show
      ...
      // Force minimum values for visibility but keep original for tooltips
  ...
  if (loading) {
    return <div className="fundingInsights">Loading intern data...</div>
  }
