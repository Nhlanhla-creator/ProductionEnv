...
export function AdvisorInsights() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch SME Advisor Applications for most requested skills
  ...
  // Process appointment success rate from AdvisoryMatches > matchPercentage
  const processAppointmentSuccessRate = () => {
    const matchCategories = {
      "Poor Matches (<50%)": 0,
      "Moderate Matches (51-80%)": 0,
      "Great Matches (80%+)": 0
    };
    
    console.log("Processing appointment success rate for", advisoryMatchesData.length, "matches");
    
    let totalMatches = 0;
    
    // Categorize matches based on matchPercentage
    advisoryMatchesData.forEach(match => {
      if (match.matchPercentage !== undefined && match.matchPercentage !== null) {
        ...
        if (!isNaN(matchRate)) {
          ...
          if (matchRate < 50) {
            matchCategories["Poor Matches (<50%)"] += 1;
          } else if (matchRate >= 50 && matchRate <= 80) {
            matchCategories["Moderate Matches (51-80%)"] += 1;
          } else if (matchRate > 80) {
            matchCategories["Great Matches (80%+)"] += 1;
          }
        }
      }
    });
    ...
    if (totalMatches > 0) {
      Object.keys(matchCategories).forEach(key => {
        matchCategories[key] = Math.round((matchCategories[key] / totalMatches) * 100);
      });
    }
    
    console.log("Appointment success rate:", matchCategories);
  ...
  // Calculate average BIG score of advisors from SmeAdvisorApplications
  const calculateAvgBigScoreOfAdvisors = () => {
    let totalBigScore = 0;
    let count = 0;
    
    console.log("Calculating average BIG score of advisors for", smeAdvisorApplicationsData.length, "SME advisor applications");
    
    smeAdvisorApplicationsData.forEach(application => {
      if (application.bigScore !== undefined && application.bigScore !== null) {
        const bigScore = typeof application.bigScore === 'number' ? application.bigScore : 
                        (typeof application.bigScore === 'string' ? parseFloat(application.bigScore) : 0);
        
        if (!isNaN(bigScore)) {
          totalBigScore += bigScore;
          count += 1;
        }
      }
    });
    
    const avgBigScore = count > 0 ? Math.round(totalBigScore / count) : 75;
    console.log("Average BIG score of advisors:", avgBigScore);
    return avgBigScore;
  };
  ...
  const insightsData = {
    ...
    activeAdvisorsCount: advisorProfilesData.length,
    averageResponseTime: 3,
    averageBigScoreOfAdvisors: calculateAvgBigScoreOfAdvisors(), // Updated from session duration
    ...
    appointmentSuccessRate: processAppointmentSuccessRate(),
  ...
  useEffect(() => {
    ...
    if (activeTab === "governance-readiness") {
      ...
      // Governance Type Distribution (Bar Chart) - UPDATED Y-AXIS
    ...
    if (activeTab === "matching-engagement") {
      ...
      // Appointment Success Rate (Donut Chart) - UPDATED
      createChart(chartRefs.appointmentSuccessRate, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: [
              brownPalette.light,      // Light brown for Poor Matches
              brownPalette.secondary,  // Medium brown for Moderate Matches
              brownPalette.accent3,    // Dark brown for Great Matches
            ],
            borderWidth: 1,
          }],
        },
    ...
    if (activeTab === "governance-effectiveness") {
      // Project Duration Distribution (Bar Chart) - UPDATED
      createChart(chartRefs.projectDurationDistribution, {
        ...
        data: {
          ...
          datasets: [{
            ...
            data: Object.values(memoizedInsights.projectDurationDistribution),
            backgroundColor: brownPalette.primary,
            borderColor: brownPalette.accent1,
            borderWidth: 1,
          }],
  ...
  if (loading) {
    return <div className="fundingInsights">Loading advisor insights...</div>;
  }
    ...
    <div className="fundingInsights">
      <div className="insightsSummary">
        ...
        <div className="insightCard">
          ...
          <div className="insightContent">
            <h3>{memoizedInsights.averageBigScoreOfAdvisors}%</h3>
            <p>Avg. BIG Score</p>
          </div>
        </div>
      </div>
