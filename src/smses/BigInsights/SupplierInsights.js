...
export function SupplierInsights() {
  ...
  const processInsightsData = () => {
    ...
    applications.forEach(app => {
      // Use the same logic as SupplierTable to get the first category
      let serviceCategory = "Not specified"

      if (app.customerProfile?.productsServices) {
        serviceCategory = getFirstCategory(app.customerProfile.productsServices)
    ...
    Object.keys(serviceCategoryCounts).forEach(category => {
      const categoryApps = applications.filter(app => {
        ...
        if (app.customerProfile?.productsServices) {
          appCategory = getFirstCategory(app.customerProfile.productsServices)
        } else if (app.originalRequest?.serviceRequested) {
