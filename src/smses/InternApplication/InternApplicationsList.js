...
// Where the Intern Matches table lives.
const MATCHES_ROUTE = "/intern-matches-page"

/* Both events are string literals rather than imports from the intern table,
   so the two files can't form an import cycle. They must match the constants
   exported there: INTERN_APPLICATION_FILTER_EVENT and
   INTERN_MATCH_RANGE_EVENT. */
const INTERN_APPLICATION_FILTER_EVENT = "intern-application-filter"
const INTERN_MATCH_RANGE_EVENT = "intern-match-range-filter"

/* The bands offered on each row's Matches cell. `test` counts them here;
   `range` is what the Intern Matches table filters on. */
const MATCH_BANDS = [
...
/* Column explanations, same idea as the Intern Matches table: an i beside each
   header, portaled to <body> so nothing clips it. */
const COLUMN_TOOLTIPS = {
  appId: "The short id for this internship advert. Hover it in the row to see the full document id.",
  application: "The internship title you advertised, with its brief description underneath.",
  matches:
    "Candidates matched to this internship. Pick a score band to see how many fall in it, then press the eye to open those matches in the Intern Matches table.",
...
const InternApplicationsList = ({
  ...
  /* applicationId -> [score, ...]. Scores are kept rather than a single count
     so the row's band picker can answer all four questions without going back
     to Firestore. */
  ...
  const fetchMatchCounts = async (userId) => {
    try {
      /* No status filter. It used to require status == "matched", but the
         match table shows every record whatever its stage — so the moment a
         candidate moved to "shortlisted" the badge count dropped while the
         table still listed the row. */
      const q = query(collection(db, "internMatchResults"), where("smeId", "==", userId))
      ...
      snapshot.forEach((d) => {
        ...
        if (!appId) return
        // Field name varies by writer, so take whichever score is present.
        const score = Number(data.finalScore ?? data.matchPercentage ?? data.matchScore) || 0
        if (!scores[appId]) scores[appId] = []
        scores[appId].push(score)
      })
  ...
  const handleDelete = async (appId) => {
    try {
      setDeleting(true)
      await deleteDoc(doc(db, "internApplicationsV2", appId))
  ...
  const handleSubmit = async (app) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "internApplicationsV2", app.id), {
        status: "submitted",
        submittedAt: serverTimestamp(),
        lastUpdated: serverTimestamp(),
      })
      setApplications((prev) => prev.map((a) => (a.id === app.id ? { ...a, status: "submitted" } : a)))
      setNavNotice(`${app.name} submitted. You'll see new candidate matches as they come in.`)
    } catch (err) {
  ...
  /* Open the Intern Matches table scoped to this application, narrowed to the
  ...
  const openMatchTable = (appId, bandKey = "all") => {
    ...
    window.dispatchEvent(new CustomEvent(INTERN_APPLICATION_FILTER_EVENT, { detail: appId }))
    window.dispatchEvent(new CustomEvent(INTERN_MATCH_RANGE_EVENT, { detail: band.range }))
  ...
  return (
    <>
      <style>{`
        ...
        /* Same header treatment as the Intern Matches table: white label on the
           dark bar, so the two tables read as one product. */
        .al-tbl thead th {
                  ...
                  return (
                    <tr key={app.id}>
                      ...
                      {/* Matches — pick a score band, then press the eye to open
                          exactly those rows in the Intern Matches table. */}
                      <td>
                        {hasAnyMatch ? (
                          <div className="al-match">
                            ...
                            <button
                              ...
                              title={
                                bandTotal === 0
                                  ? `No candidates in ${bandOf(bandKey).label.toLowerCase()} yet`
                                  : `Open the Intern Matches table — ${bandTotal} ${
                                      bandTotal === 1 ? "candidate" : "candidates"
