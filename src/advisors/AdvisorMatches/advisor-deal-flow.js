...
export function AdvisorDealFlowPipeline({ onStageClick }) {
  ...
  // REPLACE // REPLACE entire fetchStageCounts
const fetchStageCounts = async () => {
  ...
  try {
    // ✅ Correct collection — capital A capital A, matches AdvisorTable
    ...
    console.log("Pipeline: total docs found:", appSnapshot.size)

    appSnapshot.forEach((doc) => {
      const data = doc.data()
      const status = data.status || data.pipelineStage || "New Match"
      console.log("Pipeline doc status:", status)

      if (status in counts) {
        counts[status]++
      } else {
        // Statuses like "Contacted", "Shortlisted" etc fall back to New Match
        counts["New Match"]++
      }
    })
...
const stages = [
  ...
  // REPLACE handleStageClick (update the rejection query to match new field/value)
const handleStageClick = async (stage) => {
  ...
  if (stage.id === "Deal Declined" && stage.showRejectionInfo) {
    const auth = getAuth()
    const user = auth.currentUser
    if (!user) return
    ...
    if (!snapshot.empty) {
      ...
      setRejectionDetails({
        ...
        date: new Date(app.updatedAt?.toDate?.() ?? app.updatedAt).toLocaleDateString(),
