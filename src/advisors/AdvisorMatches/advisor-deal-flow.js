...
import {
  ...
  applyStageCustomization, DEFAULT_STAGE_ACTIONS, PIPELINE_REFRESH_EVENT,
...
// dashboard layout establishes a containing block.
...
// icon elements (advisorStageConfig.js stays icon-library agnostic).
const ICONS = {
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-16 px-4">
    ...
    <p className="text-sm text-[#7d5a50] text-center max-w-sm leading-relaxed">
      Apply to businesses that match your advisory expertise to start building your pipeline.
    </p>
  </div>
);
...
const STAGE_ACTION_FIELDS = [
  ...
  { key: "showTermSheet", label: "Terms/agreement upload" },
];
...
const StageCustomizePanel = ({ stages, customization, onChange, onClose, programmeType, setProgrammeType }) => {
  ...
  // Resync local editing state whenever the engagement type changes while the
    ...
    <PopupPortal>
      <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#4a352f]/40 backdrop-blur-sm font-sans" onClick={onClose}>
        <div className="bg-white rounded-3xl shadow-2xl border border-[#e6d7c3] w-[540px] max-h-[85vh] overflow-y-auto p-7" onClick={(e) => e.stopPropagation()}>
          ...
          {setProgrammeType && (
            <div className="mb-5">
              <label className="block text-xs font-semibold text-[#4a352f] mb-2">Engagement Type</label>
              <select
                value={programmeType}
                ...
                title="Choose the stage sequence for this engagement type"
              >
              ...
              <p className="text-[11px] text-[#a89482] mt-1.5">
                Each engagement type keeps its own renames, hidden stages, order, and custom stages — switching here never changes another type's setup.
              </p>
            </div>
          )}
...
// ─── Main Component ────────────────────────────────────────────────────────────
export function AdvisorDealFlowPipeline({
  onStageClick,
  showFilter = true,
  className = ""
}) {
  ...
  // Switching engagement types loads *that type's own* saved customization (or
  ...
  // ─── Counts ────────────────────────────────────────────────────────────────
  // Every application resolves through the same status→stage mapping the table
  // uses, so a renamed or custom stage is counted correctly rather than falling
  // through to "New Match" as it did with the old fixed key lookup.
  const fetchCounts = useCallback(async () => {
    ...
    try {
      ...
      const fallbackId = STAGES.find((s) => s.id === "newMatch")?.id || liveStages[0]?.id;
      snapshot.forEach((appDoc) => {
        ...
        else if (result[fallbackId] !== undefined) result[fallbackId] += 1;
      });
      setCounts(result);
    } catch (err) {
      console.error("Error fetching advisor pipeline counts:", err);
    } finally {
      setLoading(false);
    }
  }, [STAGES, liveStages]);
      ...
      <div className="flex items-center justify-between mb-7 flex-wrap gap-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#7d5a50] mt-0.5">Track your engagements, stage by stage</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => handleStageClick("newMatch")}
            ...
            style={{ borderColor: selectedStage === "newMatch" ? "#d9b98a" : "#e6d7c3" }}
            title="Show newly matched businesses"
          >
            <span className="text-3xl font-extrabold text-[#4a352f] leading-none">{counts["newMatch"] || 0}</span>
            <span className="text-[10px] text-[#7d5a50] font-semibold uppercase tracking-wide">New Matches</span>
          </button>
          {showFilter && (
            <button
...
// ─── Exports ──────────────────────────────────────────────────────────────────
export default AdvisorDealFlowPipeline;