...
import { Bell, Mail, Calendar, Settings, LogOut, User, Upload, X } from "lucide-react"
import styles from "./advisor-header.module.css"
...
function AdvisorHeader({ companyName, profileImage, setProfileImage }) {
  ...
  const GetuserName = async () => {
    try {
      ...
      const userDocRef = doc(db, "advisorProfiles", auth.currentUser.uid);
      const userDocSnap = await getDoc(userDocRef);

      if (userDocSnap.exists()) {
        const data = userDocSnap.data().formData;
        setUserName(
      ...
      <div className={styles["header-right"]}>
        <div className={styles["header-icons"]}>
          ...
          <div className={styles["icon-wrapper"]}>
            ...
            >
              <Mail size={20} />
              {unreadMessages > 0 && <span className={styles["notification-badge"]}>{unreadMessages}</span>}
            </button>
          </div>
        </div>
              ...
              <div className={styles["dropdown-header"]}>
                <div className={styles["profile-info-large"]}>
                  ...
                  <div>
                    ...
                    <p className={styles["profile-role"]}>Business Advisor</p>
                  </div>
                </div>
              </div>
