...
const sectionValidations = {
  personalProfessionalOverview: (data) => {
    ...
    const wordCount = data.briefBio?.trim().split(/\s+/).filter((word) => word.length > 0).length || 0
  ...
  selectionCriteria: (data) => {
    if (!data) return false
    const requiredFields = ["preferredAdvisorRole", "advisorySupportType", "smeStageFit", "timeCommitment", "compensationModel"]
    for (const field of requiredFields) {
      if (
        data[field] === undefined || data[field] === null ||
        (typeof data[field] === "string" && data[field].trim() === "") ||
...
const onboardingSteps = [
  { title: "Welcome to Advisor Profile", content: "This profile will help us understand your expertise and match you with SMEs that need your advisory support.", icon: "👋" },
  ...
  { title: "Step 2: Fill in Your Details", content: "Complete each section with accurate information about your background, expertise, and preferences.", icon: "📋" },
  { title: "Step 3: Upload Documents", content: "Upload required documents including ID, CV, certifications, and reference letters.", icon: "📄" },
...
export default function AdvisorProfile() {
  ...
  const checkDeclarationConsent = (data) => {
    ...
    return declarationConsent.codeOfConduct === true && declarationConsent.dataSharingConsent === true && declarationConsent.availabilityConfirmation === true
  }
  ...
  const handleEditProfile = () => {
    setShowSummary(false); setProfileSubmitted(false); setActiveSection("personalProfessionalOverview"); window.scrollTo(0, 0)
  }
  ...
  const renderActiveSection = () => {
    ...
    try {
      switch (activeSection) {
        ...
        case "personalProfessionalOverview": return <PersonalProfessionalOverview {...commonProps} />
        case "selectionCriteria": return <SelectionCriteria {...commonProps} />
        case "contactDetails": return <ContactDetails {...commonProps} />
        case "professionalCredentials": return <ProfessionalCredentials {...commonProps} />
        case "requiredDocuments": return <RequiredDocuments {...commonProps} />
  ...
  const saveDataToFirebase = async (section = null, includingSubmissionStatus = false) => {
    try {
      ...
      console.error("Error saving to Firebase:", err); throw err
    }
  }
  ...
  const handleSubmitProfile = async () => {
    try {
      ...
      if (!allSectionsValid) {
        ...
        setSectionLoading(false); return
      }
      setProfileSubmitted(true)
      await saveDataToFirebase(null, true)
      await new Promise((resolve) => setTimeout(resolve, 1200))
      const hasSeenCongratulationsPopup = localStorage.getItem(getUserSpecificKey("hasSeenAdvisorCongratulationsPopup")) === "true"
  ...
  const handleNavigateToMatches = () => navigate("/advisor-matches")
  ...
  if (showSummary && !showCongratulationsPopup) return <AdvisorProfileSummary data={formData} onEdit={handleEditProfile} />
          ...
          <div className="congratulations-popup">
            ...
            <div className="popup-content">
              ...
              <div className="popup-buttons-group">
                ...
                <button className="btn btn-secondary" onClick={handleNavigateToMatches}>View SME Matches</button>
              </div>
            </div>
          </div>
        </div>
