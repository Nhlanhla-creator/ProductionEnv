...
const sendMessageToChatGPT = async (message /*, apiKey not needed */) => {
  try {
    const functions = getFunctions(); // pass region if needed: getFunctions(undefined, "us-central1")
    const run = httpsCallable(functions, "generateInternInsights");
    const resp = await run({ prompt: message });
    const content = resp?.data?.content;
    if (!content) throw new Error("Empty response from intern insights function.");
    return content;
  } catch (err) {
    console.error("Callable error:", err);
    throw new Error(err?.message || "Failed to run intern insights.");
  }
};
