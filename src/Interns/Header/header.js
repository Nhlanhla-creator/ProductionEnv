...
function InternHeader({ companyName, profileImage, setProfileImage }) {
  ...
  const ROLE_OPTIONS = ["Investor", "SMSEs", "Advisors", "Accelerators", "Intern"];
        ...
        const fetchSenderNames = async () => {
          ...
          const messagesWithNames = await Promise.all(
            limitedDocs.map(async (doc) => {
              ...
              try {
                const senderDoc = await getDoc(doc(db, "MyuniversalProfiles", fromUID));
                if (senderDoc.exists()) {
  ...
  const GetuserName = async () => {
    try {
      ...
      const userDocRef = doc(db, "internProfiles", auth.currentUser.uid);
      const userDocSnap = await getDoc(userDocRef);

      if (userDocSnap.exists()) {
        setUserName(
          userDocSnap.data().formData?.personalOverview?.fullName ||
