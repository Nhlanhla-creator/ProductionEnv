...
function Subscriptions() {
  ...
  // Association subscription data (placeholder)
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case "trial": return `${styles.statusBadge} ${styles.statusTrial}`
  ...
  const getTierBadge = (tier, userType) => {
    if (tier === "basic" || tier === "discover") return `${styles.tierBadge} ${styles.tierBasic}`
    if (tier === "standard" || tier === "engage") return `${styles.tierBadge} ${styles.tierStandard}`
    if (tier === "premium" || tier === "partner") return `${styles.tierBadge} ${styles.tierPremium}`
    return styles.tierBadge
  }

  const getUserTypeIcon = (userType) => {
    switch(userType) {
      case "sme": return <Building2 size={16} />
      case "investor": return <TrendingUp size={16} />
            ...
            <div className={styles.detailsSection}>
              <div className={styles.detailsGrid}>
                <div className={styles.detailCard}>
                  ...
                  <div className={styles.detailItem}>
                    <span>Tier:</span>
                    <span className={getTierBadge(subscription.tier, subscription.userType)}>{subscription.tier}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span>Status:</span>
                ...
                {subscription.userType === "sme" ? (
                  <>
                    <div className={styles.usageCard}>
                      <div className={styles.usageIcon}><Target size={24} /></div>
                      <div className={styles.usageValue}>{subscription.usage.bigScoreUpdates} / {subscription.usage.maxBigScoreUpdates}</div>
                      <div className={styles.usageLabel}>BIG Score Updates</div>
                    </div>
                    <div className={styles.usageCard}>
                      <div className={styles.usageIcon}><Eye size={24} /></div>
                      <div className={styles.usageValue}>{subscription.usage.profileViews}</div>
                      <div className={styles.usageLabel}>Profile Views</div>
                    </div>
                    <div className={styles.usageCard}>
                      <div className={styles.usageIcon}><Users size={24} /></div>
                  ...
                  <>
                    ...
                    {subscription.usage.dealRoomsCreated !== undefined && (
                      <div className={styles.usageCard}>
                        <div className={styles.usageIcon}><Package size={24} /></div>
                    ...
                    <div className={styles.usageCard}>
                      <div className={styles.usageIcon}><Zap size={24} /></div>
          ...
          <div className={styles.tabContent}>
            <div className={styles.paymentsSection}>
              ...
              {subscription.paymentHistory.length > 0 ? (
                ...
                <div className={styles.emptyState}><CreditCard size={48} /><p>No payment history available</p></div>
              )}
            </div>
          </div>
        )
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.headerContent}>
          ...
          <p className={styles.subtitle}>Manage all member subscription plans and billing</p>
        </div>
      ...
      <div className={styles.userTypeStats}>
        <div className={styles.userTypeStat}><Building2 size={18} /><span>{stats.smeSubscriptions} SMEs</span></div>
        <div className={styles.userTypeStat}><TrendingUp size={18} /><span>{stats.investorSubscriptions} Investors</span></div>
        <div className={styles.userTypeStat}><Award size={18} /><span>{stats.sponsorSubscriptions} Sponsors</span></div>
      </div>
          ...
          <tbody>
            {currentSubscriptions.map((subscription) => (
              <tr key={subscription.id}>
                ...
                <td><span className={getTierBadge(subscription.tier, subscription.userType)}>{subscription.tier}</span></td>
                <td className={styles.priceCell}>{subscription.price}</td>
                <td><span className={getStatusBadge(subscription.status)}>{subscription.status}</span></td>
                <td>{formatDate(subscription.nextBillingDate)}</td>
                <td>
                  <div className={styles.actions}>
                    <button className={styles.actionBtn} onClick={() => handleAction("view", subscription)} title="View Details"><Eye size={16} /></button>
                    <button className={styles.actionBtn} onClick={() => handleAction("edit", subscription)} title="Edit"><Edit size={16} /></button>
                    {subscription.status !== "cancelled" && (
                      <button className={styles.actionBtn} onClick={() => handleAction("cancel", subscription)} title="Cancel Subscription"><Trash2 size={16} /></button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      <div className={styles.pagination}>
        ...
        <div className={styles.paginationControls}>
          ...
          <span className={styles.pageNumber}>{currentPage} of {totalPages}</span>
          <button
        ...
        <div className={styles.modalOverlay} onClick={() => setShowViewModal(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalTabs}>
              <button className={`${styles.tab} ${activeTab === "details" ? styles.tabActive : ""}`} onClick={() => setActiveTab("details")}><Package size={16} />Subscription Details</button>
              <button className={`${styles.tab} ${activeTab === "usage" ? styles.tabActive : ""}`} onClick={() => setActiveTab("usage")}><Target size={16} />Usage & Stats</button>
              <button className={`${styles.tab} ${activeTab === "payments" ? styles.tabActive : ""}`} onClick={() => setActiveTab("payments")}><CreditCard size={16} />Payment History</button>
            </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setShowEditModal(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.closeButton} onClick={() => setShowEditModal(false)}><X size={20} /></button>
            </div>
            <div className={styles.modalBody}>
              <div className={styles.editForm}>
                ...
                <div className={styles.formActions}>
                  ...
                  <button className={styles.saveButton} onClick={handleEditSave}><Save size={16} />Save Changes</button>
                </div>
              </div>
            </div>
          </div>
