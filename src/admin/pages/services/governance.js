...
const ADMIN_COLLECTION = 'admin_content';
...
const getDocId = (userId, path) => {
  return `${userId}_${path.join('_').toLowerCase().replace(/\s+/g, '_').replace(/&/g, 'and')}`;
};
...
export const uploadFile = async (path, file) => {
  try {
    ...
    const filePath = `admin/${user.uid}/${path.join('/')}/${fileName}`;
    ...
    const docRef = doc(db, ADMIN_COLLECTION, docId);
    ...
    console.log('✅ Uploaded admin file:', file.name);
...
export const addFileToCollection = async (path, file) => {
  try {
    ...
    console.log('✅ Added admin file:', file.name);
    return { success: true, url: downloadURL };
  } catch (error) {
...
export const deleteFile = async (path, fileIndex) => {
  try {
    ...
    console.log('✅ Deleted admin file:', fileToDelete.name);
    return { success: true };
  } catch (error) {
...
export const loadContent = async (path) => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.log('✅ Loaded admin content:', path.join(' > '));
      return {
        ...data,
...
export const loadAllContent = async () => {
  try {
    const user = getCurrentUser();
    const contentRef = collection(db, ADMIN_COLLECTION);
    ...
    console.log('✅ Loaded all admin content:', Object.keys(content).length);
    return content;
  } catch (error) {
...
export const deleteContent = async (path) => {
  try {
    ...
    console.log('✅ Deleted admin content:', path.join(' > '));
    return { success: true };
  } catch (error) {
