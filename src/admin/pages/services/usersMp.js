...
const USERS_COLLECTION = 'users_content';
...
export const uploadFile = async (path, file) => {
  try {
    ...
    const filePath = `users/${user.uid}/${path.join('/')}/${fileName}`;
    ...
    console.log('✅ Uploaded users file:', file.name);
...
export const addFileToCollection = async (path, file) => {
  try {
    ...
    console.log('✅ Added users file to collection:', file.name);
    return { success: true, url: downloadURL };
  } catch (error) {
...
export const deleteFile = async (path, fileIndex) => {
  try {
    ...
    console.log('✅ Deleted users file:', fileToDelete.name);
    return { success: true };
  } catch (error) {
...
export const loadContent = async (path) => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.log('✅ Loaded users content:', path.join(' > '));
      return {
        ...data,
...
export const loadAllContent = async () => {
  try {
    const user = getCurrentUser();
    const contentRef = collection(db, USERS_COLLECTION);
    ...
    console.log('✅ Loaded all users content:', Object.keys(content).length);
    return content;
  } catch (error) {
...
export const deleteContent = async (path) => {
  try {
    ...
    console.log('✅ Deleted users content:', path.join(' > '));
    return { success: true };
  } catch (error) {
