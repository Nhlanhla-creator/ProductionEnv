...
export const renameContent = async (oldPath, newPath) => {
  try {
    const user = getCurrentUser();
    const q = query(collection(db, TECH_COLLECTION), where('userId', '==', user.uid));
    const querySnapshot = await getDocs(q);
    ...
    for (const docSnap of querySnapshot.docs) {
      ...
      if (startsWith) {
        ...
        const newDocRef = doc(db, TECH_COLLECTION, newDocId);
        ...
        const oldDocRef = doc(db, TECH_COLLECTION, oldDocId);
