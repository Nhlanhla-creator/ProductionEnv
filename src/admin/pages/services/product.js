...
const STRUCTURE_COLLECTION = 'product_structure';
...
export const renameContent = async (oldPath, newPath) => {
  try {
    const user = getCurrentUser();
    const q = query(collection(db, PRODUCT_COLLECTION), where('userId', '==', user.uid));
    const querySnapshot = await getDocs(q);
    ...
    for (const docSnap of querySnapshot.docs) {
      ...
      if (startsWith) {
        ...
        const newDocRef = doc(db, PRODUCT_COLLECTION, newDocId);
        ...
        const oldDocRef = doc(db, PRODUCT_COLLECTION, oldDocId);
