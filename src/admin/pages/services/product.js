...
export const uploadFile = async (path, file) => {
  try {
    ...
    const filePath = `product/${user.uid}/${path.join('/')}/${fileName}`;
    ...
    console.log('✅ Uploaded product file:', file.name);
...
export const addFileToCollection = async (path, file) => {
  try {
    ...
    console.log('✅ Added product file to collection:', file.name);
    return { success: true, url: downloadURL };
  } catch (error) {
...
export const deleteFile = async (path, fileIndex) => {
  try {
    ...
    console.log('✅ Deleted product file:', fileToDelete.name);
    return { success: true };
  } catch (error) {
...
export const loadContent = async (path) => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.log('✅ Loaded product content:', path.join(' > '));
      return {
        ...data,
...
export const loadAllContent = async () => {
  try {
    ...
    console.log('✅ Loaded all product content:', Object.keys(content).length);
    return content;
  } catch (error) {
...
export const deleteContent = async (path) => {
  try {
    ...
    console.log('✅ Deleted product content:', path.join(' > '));
    return { success: true };
  } catch (error) {
