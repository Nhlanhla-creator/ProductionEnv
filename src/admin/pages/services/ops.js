...
const OPS_COLLECTION = 'ops_content';
...
export const uploadFile = async (path, file) => {
  try {
    ...
    const filePath = `ops/${user.uid}/${path.join('/')}/${fileName}`;
    ...
    const docRef = doc(db, OPS_COLLECTION, docId);
    ...
    console.log('✅ Uploaded ops file:', file.name);
...
export const addFileToCollection = async (path, file) => {
  try {
    ...
    console.log('✅ Added ops file to collection:', file.name);
    return { success: true, url: downloadURL };
  } catch (error) {
...
export const deleteFile = async (path, fileIndex) => {
  try {
    ...
    console.log('✅ Deleted ops file:', fileToDelete.name);
    return { success: true };
  } catch (error) {
...
export const loadContent = async (path) => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.log('✅ Loaded ops content:', path.join(' > '));
      return {
        ...data,
...
export const loadAllContent = async () => {
  try {
    const user = getCurrentUser();
    const contentRef = collection(db, OPS_COLLECTION);
    ...
    console.log('✅ Loaded all ops content:', Object.keys(content).length);
    return content;
  } catch (error) {
...
export const deleteContent = async (path) => {
  try {
    ...
    console.log('✅ Deleted ops content:', path.join(' > '));
    return { success: true };
  } catch (error) {
