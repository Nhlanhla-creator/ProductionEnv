...
import * as stageConfig from "./cmfStageConfig";

// Destructure with fallbacks
const {
  PROGRAMME_TEMPLATES = { default: { label: "Default", stages: [] } },
  mapStatusToStageId = () => "matched",
  getStageColors = () => ({ color: "#1d4ed8", bgColor: "#dbeafe", borderColor: "#bfdbfe" }),
  applyStageCustomization = (stages) => stages,
  DEFAULT_STAGE_ACTIONS = {},
  loadPipelineSettings = () => ({ programmeType: "default", customization: {} }),
  savePipelineSettings = () => {},
  loadCustomizationForType = () => ({}),
} = stageConfig;
...
// dashboard layout establishes a containing block — which is what made
// tooltips render clipped behind other elements.
const PopupPortal = ({ children }) => {
  if (typeof document === "undefined") return null;
  return createPortal(children, document.body);
};

// String → component lookup, kept next to the one file that actually needs JSX
// icon elements (cmfStageConfig.js stays icon-library agnostic).
const ICONS = {
  Target, FileText, Search, Shield, AlertCircle, FileCheck, CheckCircle,
  XCircle, LogOut, Users, Layers, DollarSign, TrendingUp,
};
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-16 px-4">
    ...
    <p className="text-sm text-[#7d5a50] text-center max-w-sm leading-relaxed">
      Set your programme criteria to start matching businesses and build your pipeline.
    </p>
  </div>
);

// ─── Stage Customization Panel ────────────────────────────────────────────────
const STAGE_ACTION_FIELDS = [
  ...
  { key: "showAgreement", label: "Offer/agreement upload" },
];
...
const StageCustomizePanel = ({ stages, customization, onChange, onClose, programmeType, setProgrammeType }) => {
  ...
  // Resync local editing state whenever the programme type changes while the
  // panel is open — otherwise "Save Changes" would write the *previous* type's
  // stage order and renames into the newly-selected type's slot.
  useEffect(() => {
  ...
  // FIX: Add fallback for when programmeType is not valid
  const template = PROGRAMME_TEMPLATES?.[programmeType] || PROGRAMME_TEMPLATES?.default || { stages: [] };
  const stagesForActions = template?.stages || [];
    ...
    <PopupPortal>
      <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#4a352f]/40 backdrop-blur-sm font-sans" onClick={onClose}>
        <div className="bg-white rounded-3xl shadow-2xl border border-[#e6d7c3] w-[540px] max-h-[85vh] overflow-y-auto p-7" onClick={(e) => e.stopPropagation()}>
          ...
          {setProgrammeType && PROGRAMME_TEMPLATES && (
            <div className="mb-5">
              ...
              >
                {Object.entries(PROGRAMME_TEMPLATES || {}).map(([key, tpl]) => (
                  <option key={key} value={key}>{tpl?.label || key}</option>
                ))}
              </select>
                  ...
                  return (
                    <div key={stage.id} className="p-3 rounded-xl border border-[#e6d7c3] bg-white shadow-sm">
                      <div className="flex items-center gap-2 mb-2.5">
                        ...
                        <p className="text-xs font-semibold text-[#4a352f]">{localRenames[stage.id] || stage.name}</p>
                      </div>
...
// ─── Main Component ────────────────────────────────────────────────────────────
export function CMFDealFlowPipeline({
  onStageClick,
  smeMatches = [],
  loading = false,
  showFilter = true,
  ...
  // FIX: Use a function to initialize state properly with validation
  const [programmeType, setProgrammeTypeRaw] = useState(() => {
    try {
      const settings = loadPipelineSettings();
      const type = settings?.programmeType || "default";
      // Validate that the type exists in PROGRAMME_TEMPLATES
      return PROGRAMME_TEMPLATES?.[type] ? type : "default";
    } catch (error) {
  ...
  const [customization, setCustomization] = useState(() => {
    try {
      ...
      console.error("Error loading pipeline customization:", error);
      return {};
    }
  });
  ...
  // Switching programme types loads *that type's own* saved customization (or a
  // clean factory default if it's never been touched) immediately, rather than
  // leaving the previously-selected type's customization in state.
  const setProgrammeType = useCallback((newType) => {
    // Validate that the type exists
    const validType = PROGRAMME_TEMPLATES?.[newType] ? newType : "default";
    setProgrammeTypeRaw(validType);
    try {
      const newCustomization = loadCustomizationForType(validType);
      setCustomization(newCustomization || {});
    } catch (error) {
      console.error("Error loading customization for type:", error);
      setCustomization({});
    }
  }, []);
  ...
  // FIX: Ensure we always have a valid template
  const STAGES = useMemo(() => {
    try {
      const template = PROGRAMME_TEMPLATES?.[programmeType] || PROGRAMME_TEMPLATES?.default || { stages: [] };
      const base = template?.stages || [];
      return applyStageCustomization(base, customization);
    } catch (error) {
      console.error("Error applying stage customization:", error);
      return [];
    }
  }, [programmeType, customization]);
  ...
  const allStagesForCustomization = useMemo(() => {
    try {
      ...
      console.error("Error applying stage customization for panel:", error);
      return [];
    }
  }, [programmeType, customization]);
  ...
  // Counts resolve every row through the same status→stage mapping the table
  // uses, so a renamed or custom stage is counted correctly rather than needing
  // its own hand-written matcher.
  const counts = useMemo(() => {
    const result = {};
    for (const stage of STAGES) result[stage.id] = 0;
    for (const sme of smeMatches) {
      try {
        const stageId = mapStatusToStageId(sme.pipelineStage || sme.currentStatus, STAGES);
        if (result[stageId] !== undefined) result[stageId] += 1;
        else if (result.matched !== undefined) result.matched += 1;
      } catch (error) {
        console.error("Error mapping status to stage:", error);
        if (result.matched !== undefined) result.matched += 1;
      }
    }
    return result;
  }, [smeMatches, STAGES]);
  ...
  const renderStageCard = (stage) => {
    ...
    // Matched is the entry point every business passes through, so "Matched"
    // reads as a running total — everyone ever matched, not just whoever
    // hasn't moved on yet.
  ...
  // If STAGES is empty, show loading or empty state
  if (STAGES.length === 0) {
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-stretch overflow-x-auto pb-3 scrollbar-thin scrollbar-thumb-[#c8b6a6] scrollbar-track-transparent gap-1">
            {liveStages.map((stage, idx) => (
              <div key={stage.id} className="flex items-center">
                ...
                {/* Connector — conversion rate to the next stage, shown above
                    the arrow, right on the flow it describes. */}
            ...
            {/* Terminal outcomes live at the end of the same pipeline row — a
                slim divider marks them as endpoints, but they use the exact
                same card shape/size as the flow stages. Declined/Withdrawn get
                one shared red border wrapping the pair. */}
...
// ─── Exports ──────────────────────────────────────────────────────────────────
export default CMFDealFlowPipeline;