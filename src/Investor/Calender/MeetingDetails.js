...
const MeetingDetails = ({ meeting, onAction, onClose }) => {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      const meetingRef = doc(db, "smeCalendarEvents", meeting.id);
      ...
      const messageContent = `The meeting with ${smeName} has been marked as completed by the investor.`;
      ...
      await addDoc(collection(db, "messages"), notification);
      
      showResponseMessage('Meeting marked as completed!');
      if (onAction) {
        onAction(meeting.id, 'completed');
      }
    } catch (error) {
      console.error('Error updating meeting:', error);
      showResponseMessage('Failed to update meeting status');
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      // Send notification to SME
      const messageContent = `The meeting with ${smeName} has been cancelled by the investor.`;
      ...
      showResponseMessage('Meeting cancelled.');
      if (onAction) {
        onAction(meeting.id, 'cancelled');
      }
    } catch (error) {
      console.error('Error cancelling meeting:', error);
      showResponseMessage('Failed to cancel meeting');
    } finally {
      setIsProcessing(false);
    }
  };
              ...
              {meeting.status === 'scheduled' && (
                <>
                  ...
                  <button
                    ...
                    disabled={isProcessing}
                  >
                    {isProcessing ? 'Processing...' : 'Cancel Meeting'}
                  </button>
                </>
              )}
              {meeting.status === 'completed' && (
                <div className="completed-message">
                  This meeting has been marked as completed
                </div>
              )}
