...
const Meetings = ({ stats, setStats }) => {
  ...
  const fetchRequesterName = async (requesterId, requesterType) => {
    ...
    try {
      const requesterRef = doc(db, "MyuniversalProfiles", requesterId);
      ...
      if (requesterSnap.exists()) {
        ...
        setRequesterNames(prev => ({ ...prev, [requesterId]: name }));
        return name;
      }
      ...
      const unsubscribe = onSnapshot(q, async (snapshot) => {
        ...
        for (const docSnap of snapshot.docs) {
          ...
          if (data.smeId) {
            ...
            requesterId = data.supplierId;
          } else {
            requesterType = 'Unknown';
          }

          const requesterName = await fetchRequesterName(requesterId, requesterType);
          ...
          if (scheduledDate) {
            slots.push({
              id: `${docSnap.id}-scheduled`,
              date: scheduledDate,
              time: data.scheduledTimeSlot?.time || '',
                ...
                <DayEvents>
                  {dayMeetings.map((meeting, idx) => (
                    <CalendarEvent 
                      ...
                      title={`${meeting.title} with ${meeting.requesterName} at ${meeting.slot.date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}
                    >
                      <EventTime>
                        {meeting.slot.date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </EventTime>
