...
import { Download, FileText, ImageIcon, X, Paperclip } from "lucide-react"
...
const Messages = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const userDoc = await getDoc(doc(db, "MyuniversalProfiles", user.uid)) // Use investor collection
      if (userDoc.exists()) {
  ...
  const handleFileView = (fileUrl, fileName) => {
    ...
    if (["jpg", "jpeg", "png", "gif", "bmp", "svg", "webp"].includes(extension)) {
      ...
      // For other files (like Excel, Word), trigger download
