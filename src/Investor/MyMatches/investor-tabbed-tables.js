...
import {
  ...
  CheckCircle, ArrowUp, ArrowDown, ArrowUpDown, Trophy
} from "lucide-react";
...
// Any wording a negative terminal stage might carry
const NEGATIVE_STATUS_RE = /declin|withdraw|unsuccess|reject|not proceed|pass/i;

// Funded statuses for successful deals
const FUNDED_STATUSES = ["Funded", "Deal Complete"];

// Renders straight to <body> so `position: fixed` popups can't be trapped
...
// ─── Successful Deals Table (Investor View) ──────────────────────────────────
const SuccessfulInvestorDealsTable = ({ onCountChange }) => {
  ...
  const DEFAULT_COLUMN_ORDER = [
    "smeName", "fundingRequested", "amountApproved", "dealType", "appliedDate",
    "fundedDate", "timeToClose", "status", "sector", "location"
  ];

  const COLUMN_DEFS = {
    smeName: {
      label: "Business Name", minWidth: "180px", filter: "text",
      tooltip: "The business that received funding through your fund.",
    },
    fundingRequested: {
      label: "Funding Requested", minWidth: "132px", filter: "range", sortKey: "fundingRequestedRaw",
      tooltip: "The amount the business asked for originally.",
    },
    amountApproved: {
      label: "Amount Approved", minWidth: "132px", filter: "range", sortKey: "amountApprovedRaw",
      tooltip: "The final amount you committed to this business.",
    },
    dealType: {
      label: "Instrument", minWidth: "112px", filter: "select", type: "badge",
      tooltip: "The funding instrument — equity, debt, grant, etc.",
    },
    appliedDate: {
      label: "Date Applied", minWidth: "116px", filter: "date", type: "date",
      tooltip: "When the business submitted its application.",
    },
    fundedDate: {
      label: "Date Funded", minWidth: "120px", filter: "date", type: "date",
      tooltip: "When the funding was confirmed and closed.",
    },
    timeToClose: {
      label: "Time to Close", minWidth: "120px", filter: "text",
      tooltip: "Business days from application to funding confirmation.",
    },
    status: {
      label: "Status", minWidth: "128px", filter: "select", type: "status",
      tooltip: "The current pipeline stage — Funded or Deal Complete.",
    },
    sector: {
  ...
  const NAME_TOOLTIP = "The business that received funding. Click the eye to open the full record.";
  const ACTIONS_TOOLTIP = "Open the full record for this funded deal.";
  ...
  const DEFAULT_SORT = { key: "fundedDate", direction: "desc" };
  ...
  const fetchDeals = useCallback(async () => {
    ...
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        // Only show funded/deal complete applications
        if (!FUNDED_STATUSES.some(s => rawStatus.toLowerCase().includes(s.toLowerCase()))) continue;
        ...
        const appliedOn = data.createdAt || null;
        const fundedOn = data.dealCompletionDate || data.fundedAt || data.updatedAt || null;
        ...
        rows.push({
          ...
          amountApproved: formatCurrency(data.amountApproved || data.fundingDetails?.amountApproved || data.fundingNeeded),
          amountApprovedRaw: parseFloat((data.amountApproved || data.fundingDetails?.amountApproved || "0").toString().replace(/[^0-9.]/g, "")) || 0,
          dealType: formatLabel(data.fundingDetails?.investmentType || data.investmentType) || "Not specified",
          appliedDate: appliedOn,
          fundedDate: fundedOn,
      ...
      console.error("Error fetching funded applications:", error);
      setNotification({ type: "error", message: "Failed to load successful deals" });
      onCountChange?.(0);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = () => {
    try {
      ...
      if (processed.length === 0) { setNotification({ type: "error", message: "No successful deals to export" }); return; }

      const header = [EXPORT_HEADERS.smeName, ...visible.map((k) => EXPORT_HEADERS[k])];
      const rows = processed.map((d) => {
        const row = { [EXPORT_HEADERS.smeName]: d.smeName };
        visible.forEach((k) => {
      ...
      case "status":
        return (
          <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border"
            style={{ backgroundColor: "#E0F2F1", color: "#00695C", borderColor: "#B2DFDB" }}>
            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: "#00695C" }} />{deal[key]}
          </span>
        );
      case "date":
        return <div className="flex items-center gap-1.5"><Calendar size={14} className="text-[#7d5a50]" />{formatDate(deal[key])}</div>;
      default:
        if (key === "fundingRequested" || key === "amountApproved") return <span className="font-semibold">{deal[key]}</span>;
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-[#f5f0e1] text-[#7d5a50] border border-[#c8b6a6]">
              <Trophy size={12} /> {deals.length} successful deal{deals.length === 1 ? "" : "s"}
            </span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#7d5a50] to-[#4a352f] text-white rounded-xl text-sm font-medium hover:shadow-lg transition-all shadow-sm"
              title="Export the current filtered/sorted successful deals to Excel (.xlsx)">
          ...
          <>
            <div className="overflow-auto" style={{ maxHeight: "70vh" }}>
              <style>{`
                ...
                .isd-fit th, .isd-fit td { overflow: hidden; }
                .isd-fit td { word-break: break-word; }
              `}</style>
              <table className="border-collapse isd-fit" style={{ tableLayout: "auto" }}>
                <thead>
                  <tr className="bg-[#4a352f]">
                    <th className="isd-th py-3 px-3 relative border-r border-[#e6d7c3] sticky top-0 left-0 z-30"
                      ...
                      <div className="flex items-start gap-1 min-w-0">
                        <span className="isd-th-label">Business Name</span>
                      ...
                      return (
                        <th key={key} draggable={!resizingColumn}
                          ...
                          <div className="flex items-start gap-1 min-w-0">
                            ...
                            <span className="isd-th-label">{col.label}</span>
                    ...
                    <th className="isd-th py-3 px-3 relative text-center whitespace-nowrap sticky top-0 z-20"
                  ...
                  {paginated.length === 0 ? (
                    <tr><td colSpan={visibleColumns.length + 2} className="text-center py-20">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-lg font-semibold text-[#4a352f]">No Successful Deals</p>
                        <p className="text-sm text-[#7d5a50] max-w-sm">
                          {activeFilterCount > 0
                            ? "Clear a filter to widen the list."
                            : "Deals appear here once you mark them as Funded or Deal Complete in your pipeline."}
                        </p>
                      </div>
                    </td></tr>
                  ) : (
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start gap-1.5 flex-wrap">
                                ...
                                <button onClick={() => setSelectedDeal(deal)} className="text-[#a89482] hover:text-[#7d5a50] transition-colors flex-shrink-0 mt-0.5"
                                  aria-label={`View successful deal for ${deal.smeName}`} title="View details">
                                  <Eye size={13} />
                                </button>
                              </div>
                            </div>
          ...
          <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-[#4a352f]/40 backdrop-blur-sm font-sans p-4" onClick={() => setSelectedDeal(null)}>
            <div className="bg-white rounded-3xl shadow-2xl border border-[#e6d7c3] w-[660px] max-w-full max-h-[86vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-6">
                {[
                  { label: "Funding details", fields: [
                    ["Funding requested", selectedDeal.fundingRequested],
                    ["Amount approved", selectedDeal.amountApproved],
                  ...
                  { label: "Timeline", fields: [
                    ...
                    ["Date funded", formatDate(selectedDeal.fundedDate)],
...
const DeclinedInvestorDealsTable = ({ onCountChange }) => {
  ...
  const DEFAULT_COLUMN_ORDER = [
    "fundingRequested", "dealType", "appliedDate", "declinedDate",
    "currentStatus", "reason", "sector", "location", "smeStage", "teamSize"
  ];
  ...
  const COLUMN_DEFS = {
    ...
    dealType: {
      label: "Instrument", minWidth: "112px", filter: "select", type: "badge",
      tooltip: "The funding instrument under discussion when the application was declined.",
    },
    appliedDate: {
      label: "Date Applied", minWidth: "116px", filter: "date", type: "date",
    ...
    smeStage: {
      ...
      tooltip: "The round the business was raising when it applied.",
    },
    teamSize: {
  ...
  const ACTIONS_TOOLTIP = "Open the full record for this declined application.";
  ...
  const EXPORT_HEADERS = {
    ...
    dealType: "Instrument",
    appliedDate: "Date Applied",
    declinedDate: "Date Declined",
    currentStatus: "Outcome",
    reason: "Reason Given",
    sector: "Sector",
    location: "Location",
    smeStage: "Funding Stage",
    teamSize: "Team Size",
  };
          ...
          <>
            <div className="overflow-auto" style={{ maxHeight: "70vh" }}>
              <style>{`
                ...
                .idd-fit th, .idd-fit td { overflow: hidden; }
                .idd-fit td { word-break: break-word; }
              `}</style>
              <table className="border-collapse idd-fit" style={{ tableLayout: "auto" }}>
                <thead>
                  <tr className="bg-[#4a352f]">
...
const InvestorTabbedTables = ({ filters, stageFilter, activeTab, setActiveTab, onDealComplete }) => {
  ...
  const tab = rawTab === "portfolio" ? "successful-deals" : rawTab;
  const setTab = setActiveTab ?? setLocalTab;

  const handleDeclinedCount = useCallback((n) => setDeclinedCount(n), []);
  const handleSuccessfulCount = useCallback((n) => setSuccessfulCount(n), []);
  ...
  const TABS = [
    ...
    { id: "successful-deals", label: "Successful Deals", icon: <Trophy size={16} />, count: successfulCount },
  ...
  return (
    <div className="w-full font-sans">
      ...
      <div className="bg-white rounded-b-2xl border border-[#e6d7c3] border-t-0 shadow-lg min-h-[500px]">
        ...
        <div style={{ display: tab === "successful-deals" ? "block" : "none" }}>
          <div className="p-6">
            <SuccessfulInvestorDealsTable onCountChange={handleSuccessfulCount} />
          </div>
        </div>
        <div style={{ display: tab === "declined-deals" ? "block" : "none" }}>
          <div className="p-6">
