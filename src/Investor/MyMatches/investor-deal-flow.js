...
import {
  ...
  DEFAULT_PIPELINE_CUSTOMIZATION, PIPELINE_REFRESH_EVENT,
...
// JSX icon elements (investorStageConfig.js stays icon-library agnostic).
const ICONS = {
...
// ─── Match scoring (used only to size the "Matches" entry pool) ───────────────
const calculateHybridScore = (investorProfile, smeProfile) => {
  ...
  const smeSectors = Array.isArray(smeProfile.entityOverview?.economicSectors)
    ...
    : [];
  totalScore += investorSectors.some((inv) =>
    smeSectors.some((sme) => sme && inv && (sme.includes(inv) || inv.includes(sme)))
  ) ? 30 : 0;
  maxPossibleScore += 30;
  ...
  totalScore += investorStages.some((inv) =>
    inv && smeStage && (smeStage.includes(inv) || inv.includes(smeStage))
  ) ? 25 : 0;
  maxPossibleScore += 25;
  ...
  const smeAmount = parseFloat(smeProfile.useOfFunds?.amountRequested?.replace(/[^\d.]/g, "") || 0);
  let ticketScore = 0;
  if (smeAmount >= investorMinTicket && smeAmount <= investorMaxTicket) {
    ...
    ticketScore = Math.max(0, 25 - Math.min((distance / range) * 25, 25));
  }
  ...
  const smeInstruments = Array.isArray(smeProfile.useOfFunds?.fundingInstruments)
    ...
    : [];
  totalScore += investorInstruments.some((inv) =>
    smeInstruments.some((sme) => sme && inv && (sme.includes(inv) || inv.includes(sme)))
  ) ? 20 : 0;
  maxPossibleScore += 20;
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-16 px-4">
    ...
    <p className="text-sm text-[#7d5a50] text-center max-w-sm leading-relaxed">
      Set your investment criteria to start matching businesses and build your deal flow pipeline.
    </p>
  </div>
);
...
const STAGE_ACTION_FIELDS = [
  ...
  { key: "showTermSheet", label: "Term sheet upload" },
  { key: "showFundingDetails", label: "Funding details" },
];
...
const StageCustomizePanel = ({ stages, customization, onChange, onClose, programmeType, setProgrammeType }) => {
  ...
  // panel is open — otherwise "Save Changes" would write the *previous*
  // type's stage order and renames into the newly-selected type's slot.
  useEffect(() => {
  ...
  const stagesForActions = (PROGRAMME_TEMPLATES[programmeType] || PROGRAMME_TEMPLATES.default).stages;
...
// ─── Main Component ────────────────────────────────────────────────────────────
export function InvestorDealFlowPipeline({
  onStageClick,
  showFilter = true,
  className = ""
}) {
  ...
  // Switching programme types loads *that type's own* saved customization (or
  // a clean factory default if it's never been touched) immediately, rather
  // than leaving the previously-selected type's customization in state.
  ...
  const fetchCounts = useCallback(async () => {
    ...
    try {
      const investorProfileSnap = await getDoc(doc(db, "MyuniversalProfiles", user.uid));
      const investorProfile = investorProfileSnap.exists() ? investorProfileSnap.data().formData : null;

      // "Matches" is the entry pool: businesses above the match threshold that
      // haven't applied yet, so it's counted from profiles rather than
      // applications.
      if (investorProfile && result.matched !== undefined) {
        const smeSnapshot = await getDocs(collection(db, "universalProfiles"));
        let highMatchCount = 0;
        smeSnapshot.forEach((smeDoc) => {
          if (calculateHybridScore(investorProfile, smeDoc.data()) > 50) highMatchCount += 1;
        });
        result.matched = highMatchCount;
      }

      const appSnapshot = await getDocs(
        query(collection(db, "investorApplications"), where("funderId", "==", user.uid))
      );

      const applicationId = STAGES.find((s) => s.id === "application")?.id || liveStages[0]?.id;
      appSnapshot.forEach((appDoc) => {
        const data = appDoc.data();
        const raw = data.pipelineStage || data.stage || data.status || "Application Received";
        const stageId = mapStatusToStageId(raw, STAGES);
        if (result[stageId] !== undefined) result[stageId] += 1;
        else if (result[applicationId] !== undefined) result[applicationId] += 1;
      });
  ...
  // The table dispatches this after a stage update lands in Firestore, so
  // counts stay in step without a page reload.
  useEffect(() => {
      ...
      <div className="flex items-center justify-between mb-7 flex-wrap gap-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#7d5a50] mt-0.5">Track your deal journey, stage by stage</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => handleStageClick("application")}
            ...
            style={{ borderColor: selectedStage === "application" ? "#d9b98a" : "#e6d7c3" }}
            title="Show applications awaiting your first response"
          >
            <span className="text-3xl font-extrabold text-[#4a352f] leading-none">{counts["application"] || 0}</span>
            <span className="text-[10px] text-[#7d5a50] font-semibold uppercase tracking-wide">New Applications</span>
          </button>
          {showFilter && (
            <button
...
// ─── Exports ──────────────────────────────────────────────────────────────────
export default InvestorDealFlowPipeline;