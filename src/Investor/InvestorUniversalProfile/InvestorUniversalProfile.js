...
const validateFundManageOverview = (data) => {
  const errors = []
  if (!data.registeredName?.trim()) errors.push("Registered name is required.")
  if (!data.registrationNumber?.trim()) errors.push("Registration number is required.")
  if (!data.legalEntityType?.trim()) errors.push("Legal entity type is required.")
  if (!data.firmType?.trim()) errors.push("Firm type is required.")
  if (!Array.isArray(data.firmSubtype) || data.firmSubtype.length === 0) errors.push("At least one firm subtype must be selected.")
  if (!data.investorRole?.trim()) errors.push("Investor role is required.")
  if (!data.yearsInOperation || Number(data.yearsInOperation) < 0) errors.push("Years in operation must be a non-negative number.")
  if (!data.numberOfInvestmentExecutives || Number(data.numberOfInvestmentExecutives) < 0) errors.push("Number of investment executives must be a non-negative number.")
  if (!data.briefDescription?.trim()) errors.push("Brief description is required.")
  if (!data.portfolioCompanies?.trim()) errors.push("Portfolio companies field is required.")
  if (!data.numberOfInvestments || Number(data.numberOfInvestments) < 0) errors.push("Number of investments must be a non-negative number.")
  if (!data.valueDeployed?.trim()) errors.push("Value deployed is required.")
  if (!Array.isArray(data.additionalSupport) || data.additionalSupport.length === 0) errors.push("At least one additional support option must be selected.")
...
const validateContactDetails = (data) => {
  const errors = []
  if (!data.businessTel?.trim()) errors.push("Business Tel is required.")
  if (!data.businessEmail?.trim()) errors.push("Business email is required.")
  if (!data.physicalAddress?.trim()) errors.push("Physical address is required.")
  if (!data.postalAddress?.trim()) errors.push("Postal address is required.")
  ...
  requiredPrimary.forEach(({ key, label }) => {
    if (!data[key] || data[key].toString().trim() === "") errors.push(`Primary contact ${label} is required.`)
  })
  const anySecondaryFilled = [
    "secondaryContactTitle", "secondaryContactName", "secondaryContactSurname",
    "secondaryContactPosition", "secondaryContactMobile", "secondaryContactEmail",
  ...
  if (anySecondaryFilled) {
    ...
    requiredSecondary.forEach(({ key, label }) => {
      if (!data[key] || data[key].toString().trim() === "") errors.push(`Secondary contact ${label} is required if adding a secondary contact.`)
    })
  }
  return errors
}
...
const onboardingSteps = [
  { title: "Welcome to Investor Universal Profile", content: "This profile will help us understand your investment preferences and match you with suitable opportunities.", icon: "👋" },
  { title: "Step 1: Read Instructions", content: "Start by reading the instructions carefully to understand what information you'll need to provide.", icon: "📝" },
  { title: "Step 2: Fill in Your Details", content: "Complete each section with accurate information about your entity, investment criteria, and preferences.", icon: "📋" },
  { title: "Step 3: Upload Documents", content: "Upload all required documents including registration documents, certificates, and fund mandates.", icon: "📄" },
  { title: "Step 4: Review & Submit", content: "Review your information in the summary page and submit when you're ready. You can always edit later.", icon: "✅" },
]
...
export default function UniversalProfile() {
  ...
  useEffect(() => {
    const requiredDocs = Array.isArray(documentsList) ? documentsList.filter(doc => doc.required).map(doc => doc.id) : []
    const uploadedDocs = formData.documentUpload || {}
    const isComplete = requiredDocs.every(docId => Array.isArray(uploadedDocs[docId]) && uploadedDocs[docId].length > 0)
  ...
  const navigateToNextSection = async () => {
    ...
    if (currentIndex < sections.length - 1) { setActiveSection(sections[currentIndex + 1].id); window.scrollTo(0, 0) }
  ...
  const navigateToPreviousSection = async () => {
    ...
    const currentIndex = sections.findIndex((s) => s.id === activeSection)
    if (currentIndex > 0) { setActiveSection(sections[currentIndex - 1].id); window.scrollTo(0, 0) }
    setSectionLoading(false)
  }

  const handleEditProfile = () => {
    setShowSummary(false); setProfileSubmitted(false); setActiveSection("fundManageOverview"); window.scrollTo(0, 0)
  }
  ...
  const renderActiveSection = () => {
    ...
    switch (activeSection) {
      ...
      case "applicationBrief": return <HowDidYouHear {...commonProps} />
      case "documentUpload": return <DocumentUpload {...commonProps} />
      case "declarationConsent": return <DeclarationConsent {...commonProps} />
  ...
  const saveDataToFirebase = async (section = null, includingSubmissionStatus = false) => {
    try {
      ...
      const dataToSave = { formData: section ? { ...uploaded } : uploaded, completedSections }
      if (includingSubmissionStatus) dataToSave.profileSubmitted = profileSubmitted
      await setDoc(docRef, dataToSave, { merge: true })
      return true
    } catch (err) {
      console.error("Error saving:", err); throw err
    }
  }

  const handleSaveSection = async () => {
    try { setSectionLoading(true); await saveDataToFirebase(activeSection) }
  ...
  const handleSubmitProfile = async () => {
    try {
      ...
      const hasSeenCongratulationsPopup = localStorage.getItem(getUserSpecificKey("hasSeenInvestorCongratulationsPopup")) === "true"
  ...
  const handleNextOnboardingStep = () => {
    if (currentOnboardingStep < onboardingSteps.length - 1) setCurrentOnboardingStep(currentOnboardingStep + 1)
    else setShowWelcomePopup(false)
  }
  const handleCloseWelcomePopup = () => setShowWelcomePopup(false)
  const handleCloseCongratulationsPopup = () => { setShowCongratulationsPopup(false); setShowSummary(true) }
  const handleNavigateToSMSEApplications = () => navigate("/investor-matches")

  if (loading) return (
    <div className="loading"><div className="spinner"></div><div className="loading-message">Loading profile data...</div></div>
  )
          ...
          <div className="validation-popup">
            ...
            <div className="popup-content">
              ...
              <ul className="list-disc pl-5 mt-2 text-sm text-red-600">
                {validationModal.messages.map((msg, idx) => <li key={idx}>{msg}</li>)}
              </ul>
              <div className="mt-4 flex justify-end">
                <button className="btn btn-primary" onClick={() => setValidationModal({ open: false, title: "", messages: [] })}>Got it</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="popup-overlay">
          <div className="congratulations-popup">
            ...
            <div className="popup-content">
              ...
              <p>You can now view your profile summary, go to your dashboard to see potential matches, or proceed to the SMSE Applications section.</p>
              <div className="popup-buttons-group">
                ...
                <button className="btn btn-secondary" onClick={handleNavigateToSMSEApplications}>Go to SMSE Applications</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className={`${styles.profileTracker} profile-tracker`}>
        <div className={`${styles.profileTrackerInner} profile-tracker-inner`}>
          {sections.map((section) => (
            ...
            >
              {section.label.split("\n").map((line, i) => (
                <span key={i} className={`${styles.trackerLabelLine} tracker-label-line`}>{line}</span>
              ))}
      ...
      <div className={`${styles.contentCard} content-card`}>
        ...
        <div className={`${styles.actionButtons} action-buttons`}>
          {activeSection !== "instructions" && (
            <button type="button" onClick={navigateToPreviousSection} className={`${styles.btn} ${styles.btnSecondary} btn btn-secondary`} disabled={sectionLoading}>
              <ChevronLeft size={16} /> Previous
            </button>
          )}
          <button type="button" onClick={handleSaveSection} className={`${styles.btn} ${styles.btnSecondary} btn btn-secondary`} disabled={sectionLoading}>
            <Save size={16} /> Save
          </button>
