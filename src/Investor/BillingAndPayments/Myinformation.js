...
import { db, auth } from "../../firebaseConfig" // Ensure this path is correct for your project
  ...
  useEffect(() => {
    const fetchAllBillingData = async () => {
      ...
      try {
        ...
        // 3. Get Profile Info (company details etc.) from MyuniversalProfiles
        ...
        const mergedData = {
          fullName:
            ...
            "",
          companyName:
            billingData.companyName || profileData?.fundManageOverview?.registeredName || userData?.company || "",
          email: billingData.email || profileData?.contactDetails?.primaryContactEmail || userData?.email || user.email,
