...
import {
  ...
  Users, Target, Briefcase, Layers, Star, Settings,
...
// dashboard layout establishes a containing block — which is what made the old
// CSS-module tooltips render clipped behind other elements.
...
// icon elements (programSponsorStageConfig.js stays icon-library agnostic).
const ICONS = {
  Target, FileText, Search, Shield, AlertCircle, FileCheck, CheckCircle,
  XCircle, LogOut, Users, Layers, Star, TrendingUp,
};
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-16 px-4">
    ...
    <h3 className="text-xl font-bold text-[#4a352f] mb-2">No Placements in Pipeline</h3>
    <p className="text-sm text-[#7d5a50] text-center max-w-sm leading-relaxed">
      Set your programme criteria to start matching candidates and build your placement pipeline.
    </p>
  </div>
);
...
const STAGE_ACTION_FIELDS = [
  { key: "showMessage", label: "Message to candidate" },
  { key: "showMeeting", label: "Meeting scheduler" },
  { key: "showAvailability", label: "Availability picker" },
  { key: "showOfferLetter", label: "Offer/agreement upload" },
  { key: "showRating", label: "Rating & feedback" },
];
...
export function ProgramSponsorDealflow({
  ...
  // ─── Counts ────────────────────────────────────────────────────────────────
  // Every application resolves through the same status→stage mapping the tables
  // use. The old version compared `app.status` against seven hard-coded
  // strings, so anything Declined or Withdrawn simply vanished from the totals.
  const fetchCounts = useCallback(async () => {
    ...
    try {
      const snapshot = await getDocs(
        query(collection(db, "internshipApplications"), where("sponsorId", "==", user.uid))
      );
      const fallbackId = STAGES.find((s) => s.id === "matched")?.id || liveStages[0]?.id;
  ...
  // A table dispatches this after a status update lands in Firestore, so counts
  // stay in step without a page reload.
  useEffect(() => {
  ...
  const getStagePercentage = useCallback((count) => {
    ...
    return ((count / totalPlacements) * 100).toFixed(1);
  }, [totalPlacements]);
            ...
            >
              <div className="bg-[#4a352f] text-[#faf7f2] text-xs rounded-2xl px-4 py-3.5 shadow-2xl">
                ...
                <div className="mt-2.5 pt-2.5 border-t border-white/10 flex items-center justify-between">
                  ...
                  <span className="text-[#a67c52] font-semibold">{count} placements</span>
                </div>
              </div>
            </div>
          </PopupPortal>
      ...
      <div className="flex items-center justify-between mb-7 flex-wrap gap-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#7d5a50] mt-0.5">Track each placement, stage by stage</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            title="Show newly matched candidates"
          >
...
// ─── Exports ──────────────────────────────────────────────────────────────────
export default ProgramSponsorDealflow;