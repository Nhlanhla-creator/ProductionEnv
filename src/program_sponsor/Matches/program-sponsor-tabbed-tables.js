...
import {
  ...
  CheckCircle, ArrowUp, ArrowDown, GraduationCap
} from "lucide-react";
...
import {
  mapStatusToStageId, getActiveStages, PIPELINE_REFRESH_EVENT,
...
const DEFAULT_COLUMN_ORDER = [
  "programName", "stipend", "currentStatus", "completionDate", "duration",
  "field", "location", "institution", "degree", "postInternship", "skills"
];

const COLUMN_DEFS = {
  programName: { label: "Program Name", minWidth: "134px", filter: "select", type: "badge" },
  stipend: { label: "Monthly Stipend", minWidth: "124px", filter: "text" },
  currentStatus: { label: "Status", minWidth: "128px", filter: "select", type: "status" },
  completionDate: { label: "Completion Date", minWidth: "124px", filter: "date", type: "date" },
  duration: { label: "Duration", minWidth: "100px", filter: "select" },
  field: { label: "Field", minWidth: "116px", filter: "select" },
  location: { label: "Location", minWidth: "104px", filter: "text" },
  institution: { label: "Institution", minWidth: "134px", filter: "select" },
  degree: { label: "Degree", minWidth: "116px", filter: "select" },
  postInternship: { label: "Post Internship", minWidth: "124px", filter: "select", type: "badge" },
  skills: { label: "Skills Developed", minWidth: "160px", filter: "text" },
};

const DEFAULT_COLUMN_VISIBILITY = {
  programName: true, stipend: true, currentStatus: true, completionDate: true,
  duration: true, field: true, location: true, postInternship: true,
  institution: false, degree: false, skills: false,
};

const EXPORT_HEADERS = {
  sme: "Intern Name", programName: "Program Name", stipend: "Monthly Stipend",
  currentStatus: "Status", completionDate: "Completion Date", duration: "Duration",
  field: "Field", location: "Location", institution: "Institution", degree: "Degree",
  postInternship: "Post Internship", skills: "Skills Developed",
};
...
const VIEWS_STORAGE_KEY = "program-sponsor-placements-views-v1";
  ...
  try {
    ...
    Object.entries(raw).forEach(([id, v]) => {
      views[id] = {
        ...
        builtin: !!v?.builtin,
        columnVisibility: { ...DEFAULT_COLUMN_VISIBILITY, ...(v?.columnVisibility || {}) },
        columnOrder: sanitizeColumnOrder(v?.columnOrder),
        sortConfig: v?.sortConfig || { ...DEFAULT_SORT },
...
const SuccessfulPlacementsTable = ({ onCountChange }) => {
  ...
  const fetchPlacements = useCallback(async () => {
    ...
    try {
      ...
      const programDuration = sponsorData.formData?.programDetails?.duration || null;

      // This previously queried `status == "Accepted"` — a value that isn't in
      // the placement status set at all, so the tab was permanently empty.
      // Rows now resolve through the shared stage config: a placement is
      // "successful" once it reaches Completed or Rated.
      const stages = getActiveStages();
      const doneIds = new Set(["completed", "rated"]);
      stages.filter((s) => s.terminal && s.group === "success").forEach((s) => doneIds.add(s.id));
      ...
      const rows = await Promise.all(
        snapshot.docs.map(async (applicationDoc) => {
          ...
          if (!doneIds.has(stageId)) return null;
          ...
          try {
            ...
            // Guarded: this comparison used to call .toLowerCase() on a
            // possibly-undefined sponsor name, which threw and took the whole
            // Promise.all down rather than just skipping the row.
            if (
              !internSponsorName || !sponsorOrgName ||
              internSponsorName.toLowerCase() !== sponsorOrgName.toLowerCase()
            ) return null;

            const startDate = affiliation.programStartDate;
            const duration = affiliation.duration || programDuration;

            return {
              id: applicationDoc.id,
              internName: data.applicantName || "Unnamed Intern",
              programName,
              stipend: monthlyStipend,
              currentStatus: stage?.name || "Completed",
              startDate,
              completionDate: addMonths(startDate, duration),
              ...
              postInternship: data.postInternship || (stageId === "rated" ? "Completed & rated" : "Completed program"),
              skills: profile.formData?.skillsInterests?.technicalSkills?.join(", ") || "Not specified",
              rating: data.sponsorRating ?? null,
              feedback: data.sponsorFeedback || null,
            };
          } catch (error) {
      ...
      setNotification({ type: "error", message: "Failed to load placements" });
      onCountChange?.(0);
    } finally {
      setLoading(false);
    }
  ...
  // Completing a placement in the matches table lands it here — refresh on that
  // signal rather than making the sponsor reload the page.
  useEffect(() => {
    const refresh = () => fetchPlacements();
  ...
  const switchToView = (id) => {
    ...
    if (!target) return;
    setViewsState((prev) => { const next = { ...prev, activeViewId: id }; persistViewsState(next); return next; });
  ...
  const processed = useMemo(() => {
    ...
    if (filters.__name__?.trim()) {
      ...
      result = result.filter((p) => p.internName.toLowerCase().includes(q));
    }
    ...
    Object.entries(COLUMN_DEFS).forEach(([key, col]) => {
      ...
      if (col.filter === "select" && Array.isArray(value) && value.length) {
        result = result.filter((p) => value.includes((p[key] || "").toString()));
      } else if (col.filter === "date" && Array.isArray(value)) {
        const [from, to] = value;
        if (from) result = result.filter((p) => { const x = toDate(p[key]); return x && x >= new Date(from); });
        if (to) result = result.filter((p) => { const x = toDate(p[key]); return x && x <= new Date(new Date(to).setHours(23, 59, 59, 999)); });
      } else if (col.filter === "text" && value.toString().trim()) {
        const q = value.toString().toLowerCase().trim();
        result = result.filter((p) => (p[key] || "").toString().toLowerCase().includes(q));
      }
    });
    ...
    if (sortConfig?.key) {
      ...
      const field = isName ? "internName" : sortConfig.key;
  ...
  const selectOptions = (key) =>
    [...new Set(placements.map((p) => (p[key] || "").toString()).filter((v) => v && v !== "Not specified"))].sort();
  ...
  const handleExport = () => {
    try {
      ...
      if (processed.length === 0) { setNotification({ type: "error", message: "No placements to export" }); return; }
      const header = [EXPORT_HEADERS.sme, ...visible.map((k) => EXPORT_HEADERS[k])];
      const rows = processed.map((p) => {
        const row = { [EXPORT_HEADERS.sme]: p.internName };
        visible.forEach((k) => { row[EXPORT_HEADERS[k]] = COLUMN_DEFS[k].type === "date" ? formatDate(p[k]) : (p[k] ?? ""); });
        return row;
      });
      ...
      XLSX.writeFile(workbook, `successful-placements-${new Date().toISOString().split("T")[0]}.xlsx`);
  ...
  return (
    <div className="w-full space-y-4 font-sans">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-[#f5f0e1] text-[#7d5a50] border border-[#c8b6a6]">
              <GraduationCap size={12} /> {placements.length} placement{placements.length === 1 ? "" : "s"}
            </span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#7d5a50] to-[#4a352f] text-white rounded-xl text-sm font-medium hover:shadow-lg transition-all shadow-sm"
              title="Export the current filtered/sorted placements to Excel (.xlsx)">
        ...
        {loading ? (
          ...
          <>
            <div className="overflow-auto" style={{ maxHeight: "70vh" }}>
              <style>{`
                .psp-th { color: #faf7f2 !important; line-height: 1.1; font-size: 0.75rem !important; font-weight: 600 !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; font-family: inherit !important; vertical-align: top !important; }
                .psp-th-draggable { cursor: grab; }
                .psp-th-draggable:active { cursor: grabbing; }
                ...
                .psp-th-label { flex: 1 1 auto; min-width: 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; white-space: normal; overflow-wrap: break-word; line-height: 1.2; text-align: left; }
              ...
              <table className="border-collapse bigt-fit" style={{ tableLayout: "auto" }}>
                <thead>
                  <tr className="bg-[#4a352f]">
                    <th className="psp-th py-3 px-3 relative border-r border-[#e6d7c3] sticky top-0 left-0 z-30"
                      style={{ backgroundColor: "#4a352f", ...widthStyle("__name__", "180px", "210px") }}>
                      <div className="flex items-start gap-1 min-w-0">
                        <button onClick={() => toggleSort("__name__")} className="psp-th-label hover:text-white transition-colors">Intern Name</button>
                    ...
                    {visibleColumns.map((key) => {
                      ...
                      return (
                        <th key={key} draggable={!resizingColumn}
                          ...
                          className={`psp-th psp-th-draggable py-3 px-3 relative border-r border-[#e6d7c3] sticky top-0 z-20 select-none transition-opacity ${draggedColumn === key ? "opacity-40" : ""}`}
                          ...
                          <div className="flex items-start gap-1 min-w-0">
                            ...
                            <button onClick={() => toggleSort(key)} className="psp-th-label hover:text-white transition-colors">{col.label}</button>
                    ...
                    <th className="psp-th py-3 px-3 relative text-center whitespace-nowrap sticky top-0 z-20" style={{ minWidth: "110px", backgroundColor: "#4a352f" }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={visibleColumns.length + 2} className="text-center py-20">
                      <div className="flex flex-col items-center gap-4">
                        <div className="w-20 h-20 rounded-full bg-[#f5f0e1] flex items-center justify-center">
                          <GraduationCap size={32} className="text-[#7d5a50] opacity-50" />
                        </div>
                        <p className="text-lg font-semibold text-[#4a352f]">No Completed Placements Yet</p>
                        <p className="text-sm text-[#7d5a50] max-w-sm">
                          {activeFilterCount > 0
                            ? "Clear a filter to widen the list."
                            : "Placements appear here once they reach the Completed or Rated stage."}
                        </p>
                      </div>
                    </td></tr>
                  ) : (
                    paginated.map((p) => (
                      <tr key={p.id} className="border-b border-[#f0e6d9] transition-all"
                        style={{ backgroundColor: hoveredRow === p.id ? "#fdf8f4" : undefined }}
                        ...
                        <td className={`${ds.cell} ${ds.fontSize} text-[#4a352f] sticky left-0 border-r border-b border-[#e6d7c3] z-10 transition-colors`}
                          style={{ ...widthStyle("__name__", "180px", "210px"), backgroundColor: hoveredRow === p.id ? "#fdf8f4" : "#ffffff" }}>
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start gap-1.5 flex-wrap">
                                <span className={`${ds.fontSize} leading-snug text-[#4a352f]`}>{p.internName}</span>
                                <button onClick={() => setSelected(p)} className="text-[#a89482] hover:text-[#7d5a50] transition-colors flex-shrink-0 mt-0.5"
                                  aria-label={`View placement details for ${p.internName}`} title="View details">
                                  <Eye size={13} />
                                </button>
                              </div>
                            </div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-[#e6d7c3] bg-[#faf7f2] rounded-b-2xl flex-wrap gap-3">
              <div className="flex items-center gap-4">
                <span className="text-sm text-[#4a352f]">
                  Showing {Math.min((currentPage - 1) * pageSize + 1, processed.length)}-{Math.min(currentPage * pageSize, processed.length)} of {processed.length} Placements
                </span>
      ...
      {headerFilterOpen && (() => {
        ...
        const col = isName ? { label: "Intern Name", filter: "text" } : COLUMN_DEFS[key];
        if (!col) return null;
        const value = filters[key];
              ...
              {col.filter === "select" && (
                <div className="flex flex-wrap gap-1.5 max-h-[200px] overflow-y-auto">
                  ...
                  {selectOptions(key).map((opt) => {
                    const selectedOpt = Array.isArray(value) && value.includes(opt);
                    return (
                      <button key={opt} onClick={() => setValue(selectedOpt ? value.filter((v) => v !== opt) : [...(value || []), opt])}
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${selectedOpt ? "bg-[#7d5a50] text-white" : "bg-[#f5f0e1] text-[#4a352f] hover:bg-[#e6d7c3]"}`}>
                        {opt}
                      </button>
                    );
                  })}
      ...
      {selected && (
        <PopupPortal>
          <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-[#4a352f]/40 backdrop-blur-sm font-sans p-4" onClick={() => setSelected(null)}>
            <div className="bg-white rounded-3xl shadow-2xl border border-[#e6d7c3] w-[640px] max-w-full max-h-[86vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="bg-gradient-to-br from-[#4a352f] to-[#7d5a50] p-5 text-white sticky top-0 z-10 flex items-center justify-between">
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-[#f5f0e1] uppercase tracking-wider">Placement</p>
                  <h3 className="text-lg font-bold mt-0.5 truncate">{selected.internName}</h3>
                  <p className="text-xs text-[#e6d7c3] mt-0.5">{selected.field} · {selected.location}</p>
                </div>
                <button onClick={() => setSelected(null)} className="text-white/70 hover:text-white p-1 flex-shrink-0"><X size={20} /></button>
              </div>

              <div className="p-6 space-y-6">
                {[
                  { label: "Programme", fields: [
                    ["Program name", selected.programName],
                    ["Monthly stipend", selected.stipend],
                    ["Duration", selected.duration],
                    ["Status", selected.currentStatus],
                  ]},
                  { label: "Timeline", fields: [
                    ["Start date", formatDate(selected.startDate)],
                    ["Completion date", formatDate(selected.completionDate)],
                    ["Post internship", selected.postInternship],
                    ["Rating", selected.rating != null ? `${selected.rating} / 5` : "Not rated"],
                  ]},
                  { label: "Intern", fields: [
                ...
                <div className="px-4 py-3 rounded-xl bg-[#faf7f2] border border-[#e6d7c3]">
                  <p className="text-[11px] uppercase tracking-wide text-[#a89482] font-semibold mb-1">Skills developed</p>
                  <p className="text-sm text-[#4a352f] leading-relaxed">{selected.skills}</p>
                </div>

                {selected.feedback && (
                  <div className="px-4 py-3 rounded-xl bg-[#faf7f2] border border-[#e6d7c3]">
                    <p className="text-[11px] uppercase tracking-wide text-[#a89482] font-semibold mb-1">Final feedback</p>
                    <p className="text-sm text-[#4a352f] leading-relaxed">{selected.feedback}</p>
                  </div>
                )}
              </div>
            </div>
...
const ProgramSponsorTabbedTables = ({ filters, stageFilter, loading }) => {
  ...
  // My Matches was hard-coded to 24. It's fed by the table now.
  const handleMatchesCount = useCallback((n) => setMatchesCount(n), []);
  const handlePlacementsCount = useCallback((n) => setPlacementsCount(n), []);

  const TABS = [
    { id: "my-matches", label: "My Matches", icon: <Users size={16} />, count: matchesCount },
    { id: "successful-placements", label: "Successful Placements", icon: <Trophy size={16} />, count: placementsCount },
  ];
    ...
    <div className="w-full font-sans">
        ...
        {TABS.map(({ id, label, icon, count }) => {
          ...
          return (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex-1 min-w-[160px] flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-semibold transition-all duration-300 whitespace-nowrap ${
      ...
      <div className="bg-white rounded-b-2xl border border-[#e6d7c3] border-t-0 shadow-lg min-h-[500px]">
        {activeTab === "my-matches" && (
          <div className="p-6">
            <ProgramSponsorInternTable filters={filters} stageFilter={stageFilter} onCountChange={handleMatchesCount} />
          </div>
        )}
        {activeTab === "successful-placements" && (
          <div className="p-6">
            <SuccessfulPlacementsTable onCountChange={handlePlacementsCount} />
          </div>
        )}
      </div>
    </div>
