...
const BillingInfoProgramSponsor = () => {
  ...
  useEffect(() => {
    const fetchAllBillingData = async () => {
      ...
      try {
        ...
        // 3. Get Profile Info (company details etc.) from ProgramSponsorUniversalProfile's collection
        const profileRef = doc(db, "programSponsorProfiles", user.uid)
        const profileSnap = await getDoc(profileRef)
        const profileData = profileSnap.exists() ? profileSnap.data().formData || {} : {}

        // Merge priority: billingProfiles > programSponsorProfiles > users
        const mergedData = {
          fullName:
          ...
          companyName:
            ...
            profileData?.entityOverview?.organizationName ||
            userData?.company ||
            "",
          email: billingData.email || profileData?.contactDetails?.emailAddress || userData?.email || user.email,
          ...
          postalCode: billingData.postalCode || profileData?.contactDetails?.postalCode || "",
  ...
  const styles = {
    ...
    contentWrapper: {
      ...
      boxShadow: `0 20px 60px rgba(55, 44, 39, 0.15), 0 8px 24px rgba(55, 44, 39, 0.06)`,
...
export default BillingInfoProgramSponsor