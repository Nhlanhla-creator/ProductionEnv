...
function ProgramSponsorSidebar({ companyName }) {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        setUser(currentUser)
        try {
          const userDocRef = doc(db, "programSponsorProfiles", currentUser.uid)
          const userDocSnap = await getDoc(userDocRef)
          if (userDocSnap.exists()) {
            const userData = userDocSnap.data()
            const registeredName =
              userData.formData.entityOverview?.organizationName ||
              userData.registeredName ||
              currentUser.displayName ||
              currentUser.email.split("@")[0]
            setUserName(registeredName || "Program Sponsor")
          } else {
            console.log("User document not found!")
            setUserName(currentUser.displayName || currentUser.email.split("@")[0] || "Program Sponsor")
          }
        } catch (error) {
  ...
  // Sidebar state management - FIXED to use "sidebar-collapsed" class
  useEffect(() => {
  ...
  const menuItems = [
    ...
    { id: "profile", label: "My Profile", icon: <User size={18} />, route: "/program-sponsor-profile" },
    { id: "insights", label: "BIG Insights", icon: <Lightbulb size={18} />, route: "/program-sponsor-insights" },
    { id: "matches", label: "My Matches", icon: <HeartHandshake size={18} />, route: "/program-sponsor-matches" },
    { id: "documents", label: "My Documents", icon: <File size={18} />, route: "/program-sponsor-documents" },
    { id: "messages", label: "My Messages", icon: <MessageSquare size={18} />, route: "/program-sponsor-messages" },
    { id: "calendar", label: "My Calendar", icon: <Calendar size={18} />, route: "/program-sponsor-calendar" },
    {
      id: "billing",
    ...
    { id: "settings", label: "Settings", icon: <Settings size={18} />, route: "/program-sponsor-settings" },
  ]

  // Keep submenus open when on submenu route
  useEffect(() => {
        ...
        <div className={styles.companyHeader}>
          ...
          {!isSidebarCollapsed && (
            <div className={styles.companyInfo}>
              ...
              <div className={styles.dashboardTitle}>Program Sponsor Portal</div>
            </div>
          )}
        </div>
          ...
          <nav className={styles.menu} role="navigation">
            {menuItems.map((item, index) => (
              ...
              >
                ...
                {/* Submenu Items */}
                {item.hasSubmenu && expandedMenus[item.id] && !isSidebarCollapsed && (
